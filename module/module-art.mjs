...
export class ModuleArt {
  ...
  static getArtModules() {
    const settings = game.settings.get("rotv", "moduleArtConfiguration");
    const unsorted = [];
    const configs = [{
      id: game.system.id,
      label: game.system.title,
      mapping: "systems/rotv/json/fa-token-mapping.json",
      priority: settings.rotv?.priority ?? CONST.SORT_INTEGER_DENSITY,
      credit: `
        <em>
          Token artwork by
    ...
    for ( const module of game.modules ) {
      ...
      if ( !mapping ) continue;
      const config = { id: module.id, label: module.title, credit: flags?.["rotv-art-credit"], mapping };
      configs.push(config);
...
export class ModuleArtConfig extends FormApplication {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      template: "systems/rotv/templates/apps/module-art-config.hbs",
      popOut: true,
      width: 600,
      height: "auto"
    });
