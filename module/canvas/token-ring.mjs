 ...
 * @param {TokenRotV} token  Reference to the token containing this ring.
 */
export default class TokenRing {
  ...
  configureMesh(mesh) {
    ...
    if ( this.enabled ) {
      const tokenRingFlag = this.token.document.getFlag("rotv", "tokenRing");
  ...
  async flashColor(color, animationOptions={}) {
    ...
    const originalColor = Color.from(foundry.utils.mergeObject(
      this.token.document.getFlag("rotv", "tokenRing.colors") ?? {},
  ...
  static initialize() {
    ...
    this.tokenRingSamplerShader = CONFIG.ROTV.tokenRings.shaderClass;
  ...
  static pushToLoad(additionalSources) {
    additionalSources.push(CONFIG.ROTV.tokenRings.spriteSheet);
  ...
  static createAssetsUVs() {
    ...
    const spritesheet = TextureLoader.loader.getCache(CONFIG.ROTV.tokenRings.spriteSheet);
