...
async function enrichAward(config, label, options) {
  ...
  if ( parsed.xp ) entries.push(`
    <span class="award-entry">
      ${formatNumber(parsed.xp)} ${game.i18n.localize("ROTV.ExperiencePointsAbbr")}
    </span>
  `);

  let award = game.i18n.getListFormatter({ type: "unit" }).format(entries);
  if ( parsed.each ) award = game.i18n.format("EDITOR.ROTV.Inline.AwardEach", { award });

  block.innerHTML += `
    ${award}
    <a class="award-link" data-action="awardRequest">
      <i class="fa-solid fa-trophy"></i> ${label ?? game.i18n.localize("ROTV.Award.Action")}
    </a>
  `;
...
async function enrichCheck(config, label, options) {
  for ( let value of config.values ) {
    ...
    if ( value in CONFIG.ROTV.enrichmentLookup.abilities ) config.ability = value;
    else if ( value in CONFIG.ROTV.enrichmentLookup.skills ) config.skill = value;
    else if ( value in CONFIG.ROTV.enrichmentLookup.tools ) config.tool = value;
  ...
  const skillConfig = CONFIG.ROTV.enrichmentLookup.skills[slugify(config.skill)];
  ...
  const toolUUID = CONFIG.ROTV.enrichmentLookup.tools[slugify(config.tool)];
  ...
  let abilityConfig = CONFIG.ROTV.enrichmentLookup.abilities[slugify(config.ability)];
...
async function enrichSave(config, label, options) {
  ...
  const abilityConfig = CONFIG.ROTV.enrichmentLookup.abilities[config.ability];
...
async function enrichDamage(config, label, options) {
  ...
  for ( const value of config.values ) {
    if ( value in CONFIG.ROTV.damageTypes ) config.type = value;
    else if ( value in CONFIG.ROTV.healingTypes ) config.type = value;
  ...
  const typeConfig = CONFIG.ROTV.damageTypes[config.damageType] ?? CONFIG.ROTV.healingTypes[config.damageType];
  ...
  span.innerHTML = game.i18n.format(`EDITOR.ROTV.Inline.Damage${localizationType}`, localizationData);
  return span;
}
 ...
 *   <a class="content-link" draggable="true"
 *      data-uuid="Compendium.rotv.rules.JournalEntry.w7eitkpD7QQTB6j0.JournalEntryPage.UWw13ISmMxDzmwbd"
 ...
 *      data-tooltip="EDITOR.ROTV.Inline.ApplyStatus" aria-label="Apply Status to Selected Tokens">
...
async function enrichReference(config, label, options) {
  ...
  const type = Object.keys(config).find(k => k in CONFIG.ROTV.ruleTypes);
  if ( type ) {
    key = slugify(config[type]);
    source = foundry.utils.getProperty(CONFIG.ROTV, CONFIG.ROTV.ruleTypes[type].references)?.[key];
  } else if ( config.values.length ) {
    key = slugify(config.values.join(""));
    for ( const [type, { references }] of Object.entries(CONFIG.ROTV.ruleTypes) ) {
      source = foundry.utils.getProperty(CONFIG.ROTV, references)[key];
      if ( source ) {
  ...
  if ( isCondition && (config.apply !== false) ) {
    ...
    apply.dataset.tooltip = "EDITOR.ROTV.Inline.ApplyStatus";
...
export function createRollLabel(config) {
  const { label: ability, abbreviation } = CONFIG.ROTV.abilities[config.ability] ?? {};
  const skill = CONFIG.ROTV.skills[config.skill]?.label;
  const toolUUID = CONFIG.ROTV.enrichmentLookup.tools[config.tool];
  ...
  switch ( config.type ) {
    ...
    case "tool":
      if ( ability && (skill || tool) ) {
        label = game.i18n.format("EDITOR.ROTV.Inline.SpecificCheck", { ability, type: skill ?? tool });
      } else {
        label = ability;
      }
      if ( config.passive ) {
        label = game.i18n.format(`EDITOR.ROTV.Inline.DCPassive${longSuffix}`, { dc: config.dc, check: label });
      } else {
        if ( showDC ) label = game.i18n.format("EDITOR.ROTV.Inline.DC", { dc: config.dc, check: label });
        label = game.i18n.format(`EDITOR.ROTV.Inline.Check${longSuffix}`, { check: label });
      }
      break;
    case "concentration":
    case "save":
      if ( config.type === "save" ) label = ability;
      else label = `${game.i18n.localize("ROTV.Concentration")} ${ability ? `(${abbreviation})` : ""}`;
      if ( showDC ) label = game.i18n.format("EDITOR.ROTV.Inline.DC", { dc: config.dc, check: label });
      label = game.i18n.format(`EDITOR.ROTV.Inline.Save${longSuffix}`, { save: label });
      break;
    default:
      return "";
  }
  ...
  if ( config.icon ) {
    switch ( config.type ) {
      case "check":
      case "skill":
        label = `<i class="rotv-icon" data-src="systems/rotv/icons/svg/ability-score-improvement.svg"></i>${label}`;
        break;
      case "tool":
...
function createRollLink(label, dataset) {
  ...
  if ( game.user.isGM && (dataset.type !== "damage") && (dataset.type !== "item") ) {
    ...
    gmLink.dataset.tooltip = "EDITOR.ROTV.Inline.RequestRoll";
...
async function rollAction(event) {
  ...
  if ( (action === "roll") || !game.user.isGM ) {
    ...
    try {
      ...
      if ( !tokens.length ) {
        ui.notifications.warn("EDITOR.ROTV.Inline.Warning.NoActor", { localize: true });
        return;
      }
      ...
      for ( const token of tokens ) {
        ...
        switch ( type ) {
          case "check":
            ...
            break;
          case "concentration":
            if ( ability in CONFIG.ROTV.abilities ) options.ability = ability;
  ...
  // Roll request
    ...
    const chatData = {
      ...
      flavor: game.i18n.localize("EDITOR.ROTV.Inline.RollRequest"),
...
async function rollDamage(event) {
  ...
  const isHealing = damageType in CONFIG.ROTV.healingTypes;
  const title = game.i18n.localize(`ROTV.${isHealing ? "Healing" : "Damage"}Roll`);
  const rollConfig = {
    rollConfigs: [{
      parts: [formula],
      type: damageType
    ...
    messageData: {
      "flags.rotv": {
        targets: ItemRotV._formatAttackTargets(),
        roll: {type: "damage"}
      },
  ...
  if ( Hooks.call("rotv.preRollDamage", undefined, rollConfig) === false ) return;
  const roll = await damageRoll(rollConfig);
  if ( roll ) Hooks.callAll("rotv.rollDamage", undefined, roll);
}
...
async function useItem({ rollItemUuid, rollItemName, rollItemActor }={}) {
  ...
  if ( !item ) {
    ...
    if ( item && canvas.tokens.controlled.length ) ui.notifications.warn(
      game.i18n.format("MACRO.RotVMissingTargetWarn", {
  ...
  ui.notifications.warn(game.i18n.format("EDITOR.ROTV.Inline.Warning.NoItemOnActor", {
