...
export default class InventoryElement extends HTMLElement {
  ...
  /**
   * TODO: Remove filtering code from rotv-inventory when all sheets use item-list-controls.
   * Apply the current set of filters to the inventory list.
   * @param {FilterStateRotV} state  The filter state to apply.
   * @protected
   */
  _applyFilters(state) {
   ...
   * @param {string} id
   * @returns {ItemRotV|Promise<ItemRotV>}
   */
  getItem(id) {
   ...
   * @param {ItemRotV} item           The Item for which the context menu is activated.
  ...
  _getContextOptions(item, element) {
    ...
    if ( item.system.attunement ) {
      options.push({
        name: item.system.attuned ? "ROTV.ContextMenuActionUnattune" : "ROTV.ContextMenuActionAttune",
    ...
    if ( item.system.recharge?.value ) options.push({
      name: item.system.recharge.charged ? "ROTV.ContextMenuActionExpendCharge" : "ROTV.ContextMenuActionCharge",
  ...
  async _onCreate(target) {
    ...
    const itemData = {
      name: game.i18n.format("ROTV.ItemNew", {type: game.i18n.localize(CONFIG.Item.typeLabels[type])}),
      type,
   ...
   * @param {HTMLElement} target  Button or context menu entry that triggered this action.
   * @param {ItemRotV} item         Item to being expanded or collapsed.
   */
