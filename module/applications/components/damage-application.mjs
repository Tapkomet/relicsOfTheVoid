...
export default class DamageApplicationElement extends ChatTrayElement {
    ...
    if ( !this.targetList ) {
      ...
      div.innerHTML = `
        <label class="roboto-upper">
          ...
          <span>${game.i18n.localize("ROTV.Apply")}</span>
          ...
          <div class="wrapper">
            <div class="target-source-control">
              <button type="button" class="unbutton" data-mode="targeted" aria-pressed="false">
                <i class="fa-solid fa-bullseye" inert></i> ${game.i18n.localize("ROTV.Tokens.Targeted")}
              </button>
              <button type="button" class="unbutton" data-mode="selected" aria-pressed="false">
                <i class="fa-solid fa-expand" inert></i> ${game.i18n.localize("ROTV.Tokens.Selected")}
              </button>
            </div>
      ...
      if ( !this.chatMessage.getFlag("rotv", "targets")?.length ) this.targetSourceControl.hidden = true;
  ...
  buildTargetsList() {
    ...
    switch ( this.targetingMode ) {
      case "targeted":
        targetedTokens = (this.chatMessage.getFlag("rotv", "targets") ?? []).map(t => t.uuid);
        break;
      case "selected":
    ...
    if ( targets.length ) this.targetList.replaceChildren(...targets);
      ...
      li.innerText = game.i18n.localize(`ROTV.Tokens.None${this.targetingMode.capitalize()}`);
  ...
  buildTargetListEntry(uuid) {
    ...
    const changeSources = types.reduce((acc, {type, change, icon}) => {
      ...
      acc += `
        <button class="change-source unbutton" type="button" data-type="${type}" data-change="${change}"
          ...
          <rotv-icon src="${icon}" inert></rotv-icon>
    ...
    li.innerHTML = `
      ...
      <div class="calculated temp" data-tooltip="ROTV.HitPointsTemp">
        ${temp}
      </div>
  ...
  getChangeSourceOptions(type, change, options) {
    ...
    let label = game.i18n.format(`ROTV.DamageApplication.Change.${change.capitalize()}`, {
      type: CONFIG.ROTV.damageTypes[type]?.label ?? CONFIG.ROTV.healingTypes[type]?.label
    });
    if ( mode === "ignore" ) label = game.i18n.format("ROTV.DamageApplication.Ignoring", { source: label });
    if ( mode === "downgrade" ) label = game.i18n.format("ROTV.DamageApplication.Downgrading", { source: label });
  ...
  refreshListEntry(token, entry, options) {
    ...
    calculatedDamage.dataset.tooltip = `ROTV.${total < 0 ? "Healing" : "Damage"}`;
