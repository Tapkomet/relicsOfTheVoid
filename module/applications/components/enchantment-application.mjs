...
export default class EnchantmentApplicationElement extends HTMLElement {
  ...
  connectedCallback() {
    ...
    const spellLevel = this.chatMessage.getFlag("rotv", "use.spellLevel");
    if ( spellLevel ) {
    ...
    if ( maxTargets ) {
      ...
      this.countArea.innerHTML = game.i18n.format("ROTV.Enchantment.Enchanted", {
  ...
  async buildItemList() {
    const enchantedItems = (await EnchantmentData.appliedEnchantments(this.enchantmentItem.uuid)).map(enchantment => {
      ...
      if ( item.isOwner ) {
        ...
        control.ariaLabel = game.i18n.localize("ROTV.Enchantment.Action.Remove");
        control.dataset.action = "removeEnchantment";
        control.dataset.tooltip = "ROTV.Enchantment.Action.Remove";
    ...
    if ( enchantedItems.length ) {
      ...
      this.dropArea.innerHTML = `<p>${game.i18n.localize("ROTV.Enchantment.DropArea")}</p>`;
    }
    if ( this.countArea ) {
  ...
  async _onDrop(event) {
    ...
    const effect = this.enchantmentItem.effects.get(this.chatMessage.getFlag("rotv", "use.enchantmentProfile"));
    ...
    const concentrationId = this.chatMessage.getFlag("rotv", "use.concentrationId");
    const concentration = effect.parent.actor.effects.get(concentrationId);
    if ( concentrationId && !concentration ) {
      ui.notifications.error("ROTV.Enchantment.Warning.ConcentrationEnded", { localize: true });
      return;
    }
    if ( !game.user.isGM && concentration && !concentration.actor?.isOwner ) {
      ui.notifications.error("ROTV.EffectApplyWarningConcentration", { localize: true });
      return;
    }
