...
export default class ItemListControlsElement extends HTMLElement {
  ...
  /**
   * @typedef {object} SortModeConfigurationRotV
   * @property {string} icon
   * @property {string} label
   * @property {ItemListComparatorRotV} comparator
   */

  /**
   * @callback ItemListComparatorRotV
   * @param {object} a
   * @param {object} b
   * @returns {number}
   */
   ...
   * @type {Record<string, SortModeConfigurationRotV>}
   */
  static SORT_MODES = {
    a: {
   ...
   * @type {TabPreferencesRotV}
   */
  get prefs() {
    return game.user.getFlag("rotv", `sheetPrefs.${this.app.object.type}.tabs.${this.tab}`);
  }
  ...
  #buildHTML() {
    ...
    search.innerHTML = `
      ...
      <ul class="unlist controls">
        <li>
          <button type="button" class="unbutton filter-control" data-action="clear" data-tooltip="ROTV.FilterClear"
                  aria-label="${game.i18n.localize("ROTV.FilterClear")}">
    ...
    if ( options.length ) {
      ...
      item.innerHTML = `
        <button type="button" class="unbutton filter-control filter" data-action="filter"
                aria-label="${game.i18n.localize("ROTV.Filter")}">
  ...
  async _onToggleMode(event) {
    ...
    const current = game.user.getFlag("rotv", flag);
    let value;
    ...
    await game.user.setFlag("rotv", flag, value);
    if ( action === "group" ) {
      this._initGrouping();
      this._applyGrouping();
    } else if ( action === "sort" ) {
