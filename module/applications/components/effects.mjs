...
export default class EffectsElement extends HTMLElement {
  connectedCallback() {
    ...
    const MenuCls = this.hasAttribute("v2") ? ContextMenuRotV : ContextMenu;
   ...
   * @param {ActiveEffectRotV[]} effects         The array of Active Effect instances for which to prepare sheet data.
   * @param {object} [options={}]
   * @param {ActorRotV|ItemRotV} [options.parent]  Document that owns these active effects.
  ...
  static prepareCategories(effects, { parent }={}) {
    ...
    const categories = {
      enchantment: {
        type: "enchantment",
        label: game.i18n.localize("ROTV.Enchantment.Category.General"),
        effects: [],
        isEnchantment: true
      },
      temporary: {
        type: "temporary",
        label: game.i18n.localize("ROTV.EffectTemporary"),
        effects: []
      },
      enchantmentActive: {
        type: "activeEnchantment",
        label: game.i18n.localize("ROTV.Enchantment.Category.Active"),
        effects: [],
        isEnchantment: true
      },
      passive: {
        type: "passive",
        label: game.i18n.localize("ROTV.EffectPassive"),
        effects: []
      },
      enchantmentInactive: {
        type: "inactiveEnchantment",
        label: game.i18n.localize("ROTV.Enchantment.Category.Inactive"),
        effects: [],
        isEnchantment: true
      },
      inactive: {
        type: "inactive",
        label: game.i18n.localize("ROTV.EffectInactive"),
        effects: []
      },
      suppressed: {
        type: "suppressed",
        label: game.i18n.localize("ROTV.EffectUnavailable"),
        effects: [],
        disabled: true,
        info: [game.i18n.localize("ROTV.EffectUnavailableInfo")]
      }
    };
    ...
    for ( const e of effects ) {
      ...
      else if ( e.getFlag("rotv", "type") === "enchantment" ) categories.enchantment.effects.push(e);
    ...
    for ( const category of Object.values(categories) ) {
      category.localizationPrefix = category.isEnchantment ? "ROTV.Enchantment.Action." : "ROTV.Effect";
    }
   ...
   * @param {ActiveEffectRotV} effect  The ActiveEffect for which the context menu is activated.
   * @returns {ContextMenuEntry[]}   An array of context menu options offered for the ActiveEffect.
   * @protected
   */
  _getContextOptions(effect) {
    const isConcentrationEffect = (this.document instanceof ActorRotV) && this._app._concentration?.effects.has(effect);
    const options = [
      {
      ...
      {
        name: "ROTV.ConcentrationBreak",
        icon: '<rotv-icon src="systems/rotv/icons/svg/break-concentration.svg"></rotv-icon>',
    ...
    if ( (this.document instanceof ActorRotV) && ("favorites" in this.document.system) ) {
      const uuid = effect.getRelativeUUID(this.document);
      const isFavorited = this.document.system.hasFavorite(uuid);
      options.push({
        name: isFavorited ? "ROTV.FavoriteRemove" : "ROTV.Favorite",
  ...
  async _onToggleCondition(conditionId) {
    const existing = this.document.effects.get(staticID(`rotv${conditionId}`));
  ...
  async _onCreate(target) {
    ...
    return this.document.createEmbeddedDocuments("ActiveEffect", [{
      name: isActor ? game.i18n.localize("ROTV.EffectNew") : this.document.name,
      ...
      "flags.rotv.type": isEnchantment ? "enchantment" : undefined
    }]);
  }
  ...
  async _onClickEffectSource(event) {
    ...
    if ( !doc.testUserPermission(game.user, "LIMITED") ) {
      ui.notifications.warn("ROTV.DocumentViewWarn", { localize: true });
      return;
    }
