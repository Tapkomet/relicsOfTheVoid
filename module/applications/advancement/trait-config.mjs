...
export default class TraitConfig extends AdvancementConfig {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["rotv", "advancement", "traits", "two-column"],
      template: "systems/rotv/templates/advancement/trait-config.hbs",
      width: 640
    });
  }
  ...
  async getData() {
    ...
    context.validTraitTypes = Object.entries(CONFIG.ROTV.traits).reduce((obj, [key, config]) => {
    ...
    if ( traitConfig ) {
      ...
      context.default.title = game.i18n.localize("ROTV.TraitGenericPlural.other");
    ...
    context.selectedTraitHeader = `${CONFIG.ROTV.traits[this.trait].labels.localization}.other`;
  ...
  async _onChangeInput(event) {
    ...
    if ( (event.target.name === "configuration.mode")
      ...
      const validTraitTypes = filteredKeys(CONFIG.ROTV.traits, c => c.expertise);
