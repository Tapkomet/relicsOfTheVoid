 ...
 *
 * @param {ItemRotV} item             Item to which this advancement will be added.
...
export default class AdvancementSelection extends Dialog {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["rotv", "sheet", "advancement"],
      template: "systems/rotv/templates/advancement/advancement-selection.hbs",
      title: "ROTV.AdvancementSelectionTitle",
      width: 500,
      height: "auto"
    });
  }
  ...
  getData() {
    const context = { types: {} };
    for ( let [name, config] of Object.entries(CONFIG.ROTV.advancementTypes) ) {
    ...
    context.types = rotv.utils.sortObjectEntries(context.types, "label");
    return context;
  }
   ...
   * been resolved.
   * @param {ItemRotV} item                         Item to which the advancement should be added.
   * @param {object} [config={}]
   ...
   * @returns {Promise<AdvancementConfig|null>}   Result of `ItemRotV#createAdvancement`.
   */
  static async createDialog(item, { rejectClose=false, options={} }={}) {
    return new Promise((resolve, reject) => {
      const dialog = new this(item, {
        title: `${game.i18n.localize("ROTV.AdvancementSelectionTitle")}: ${item.name}`,
        buttons: {
          submit: {
            callback: html => {
