...
export default class ItemChoiceFlow extends ItemGrantFlow {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      template: "systems/rotv/templates/advancement/item-choice-flow.hbs"
    });
  }
  ...
  async retainData(data) {
    ...
    this.selected = new Set(data.items.map(i => foundry.utils.getProperty(i, "flags.rotv.sourceId")));
  }
  ...
  async getContext() {
    ...
    if ( !this.dropped ) {
      ...
      for ( const data of this.retainedData?.items ?? [] ) {
        const uuid = foundry.utils.getProperty(data, "flags.rotv.sourceId");
  ...
  async _onDrop(event) {
    ...
    for ( const [level, data] of Object.entries(this.advancement.value.added ?? {}) ) {
      ...
      if ( Object.values(data).includes(item.uuid) ) {
        ui.notifications.error("ROTV.AdvancementItemChoicePreviouslyChosenWarning", {localize: true});
        return null;
      }
    }
    ...
    if ( (item.system.prerequisites?.level ?? -Infinity) > this.level ) {
      ui.notifications.error(game.i18n.format("ROTV.AdvancementItemChoiceFeatureLevelWarning", {
    ...
    if ( (this.advancement.configuration.type === "spell") && spellLevel === "available" ) {
      ...
      if ( item.system.level > maxSlot ) {
        ui.notifications.error(game.i18n.format("ROTV.AdvancementItemChoiceSpellLevelAvailableWarning", {
          level: CONFIG.ROTV.spellLevels[maxSlot]
        }));
        return null;
      }
    }
  ...
  _maxSpellSlotLevel() {
    ...
    if ( spellcasting ) {
      ...
      ActorRotV.computeClassProgression(progression, this.advancement.item, { spellcasting });
      ActorRotV.prepareSpellcastingSlots(spells, spellcasting.type, progression);
    }
