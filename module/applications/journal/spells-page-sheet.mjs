...
import ItemsRotV from "../../data/collection/items-collection.mjs";
...
export default class JournalSpellListPageSheet extends JournalPageSheet {
  ...
  /** @inheritDoc */
  get template() {
    if ( this.options.displayAsTable ) return "systems/rotv/templates/journal/page-spell-list-table.hbs";
    return `systems/rotv/templates/journal/page-spell-list-${this.isEditable ? "edit" : "view"}.hbs`;
  }
    ...
    for ( const data of context.spells ) {
      ...
      switch ( context.grouping ) {
        case "level":
          ...
          section = context.sections[level] ??= { header: CONFIG.ROTV.spellLevels[level], spells: [] };
          break;
        case "school":
          const school = spell.system.school;
          section = context.sections[school] ??= { header: CONFIG.ROTV.spellSchools[school]?.label, spells: [] };
          break;
        case "alphabetical":
  ...
  async prepareSpells(grouping) {
    ...
    for ( const uuid of uuids ) {
      ...
      if ( collection && !collections.has(collection) ) {
        if ( collection instanceof ItemsRotV ) collections.set(collection, collection);
