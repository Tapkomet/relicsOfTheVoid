...
export default class CompendiumBrowser extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  static DEFAULT_OPTIONS = {
    ...
    classes: ["rotv2", "compendium-browser", "vertical-tabs"],
    tag: "form",
    window: {
      title: "ROTV.CompendiumBrowser.Title",
      minimizable: true,
      resizable: true
    },
    actions: {
  ...
  static PARTS = {
    header: {
      id: "header",
      classes: ["header"],
      template: "systems/rotv/templates/compendium/browser-header.hbs"
    },
    search: {
      id: "sidebar-search",
      classes: ["sidebar-part", "filter-element"],
      template: "systems/rotv/templates/compendium/browser-sidebar-search.hbs"
    },
    types: {
      id: "sidebar-types",
      classes: ["sidebar-part"],
      template: "systems/rotv/templates/compendium/browser-sidebar-types.hbs"
    },
    filters: {
      id: "sidebar-filters",
      classes: ["sidebar-part"],
      template: "systems/rotv/templates/compendium/browser-sidebar-filters.hbs"
    },
    results: {
      id: "results",
      classes: ["results"],
      template: "systems/rotv/templates/compendium/browser-results.hbs",
      templates: ["systems/rotv/templates/compendium/browser-entry.hbs"],
      scrollable: [""]
    },
    footer: {
      id: "footer",
      classes: ["footer"],
      template: "systems/rotv/templates/compendium/browser-footer.hbs"
    },
    tabs: {
      id: "tabs",
      classes: ["tabs", "tabs-left"],
      template: "systems/rotv/templates/compendium/browser-tabs.hbs"
    }
  };
  ...
  /**
   * @typedef {SheetTabDescriptorRotV} CompendiumBrowserTabDescriptorRotV
  ...
  /**
   * Application tabs.
   * @type {CompendiumBrowserTabDescriptorRotV[]}
   */
  static TABS = [
    {
      tab: "classes",
      label: "TYPES.Item.classPl",
      svg: "systems/rotv/icons/svg/items/class.svg",
      documentClass: "Item",
      types: ["class"]
    },
    {
      ...
      svg: "systems/rotv/icons/svg/items/subclass.svg",
      documentClass: "Item",
      types: ["subclass"]
    },
    {
      ...
      svg: "systems/rotv/icons/svg/items/race.svg",
      documentClass: "Item",
      types: ["race"]
    },
    {
      tab: "feats",
      label: "ROTV.CompendiumBrowser.Tabs.Feat.other",
      icon: "fas fa-star",
      documentClass: "Item",
      types: ["feat"]
    },
    {
      ...
      svg: "systems/rotv/icons/svg/items/background.svg",
      documentClass: "Item",
      types: ["background"]
    },
    {
      tab: "physical",
      label: "ROTV.CompendiumBrowser.Tabs.Item.other",
      svg: "systems/rotv/icons/svg/backpack.svg",
      documentClass: "Item",
      types: ["physical"]
    },
    {
    ...
    {
      tab: "monsters",
      label: "ROTV.CompendiumBrowser.Tabs.Monster.other",
      svg: "systems/rotv/icons/svg/monster.svg",
      documentClass: "Actor",
      types: ["npc"]
    },
    {
      ...
      svg: "systems/rotv/icons/svg/vehicle.svg",
      documentClass: "Actor",
      types: ["vehicle"]
    },
    {
  ...
  _configureRenderOptions(options) {
    ...
    if ( options.isFirstRender ) {
      const tab = this.constructor.TABS.find(t => t.tab === this.options.tab);
      if ( tab ) foundry.utils.setProperty(options, "rotv.browser.types", tab.types);
    }
  }
  ...
  async _prepareFooterContext(context, options) {
    ...
    context.summary = suffix ? game.i18n.format(
      `ROTV.CompendiumBrowser.Selection.Summary.${suffix}`, { max, min, value }
    ) : value;
    return context;
  }
  ...
  async _prepareSidebarContext(partId, context, options) {
    ...
    const types = foundry.utils.getProperty(options, "rotv.browser.types") ?? [];
  ...
  async _renderResult(entry, documentClass) {
    ...
    const html = await renderTemplate("systems/rotv/templates/compendium/browser-entry.hbs", context);
  ...
  changeTab(tab, group, options={}) {
    ...
    this.render({ parts: ["results", "filters", "types"], rotv: { browser: { types } } });
  }
  ...
  #adjustCheckboxStates(htmlElement) {
    for ( const groupArea of htmlElement.querySelectorAll(".type-group") ) {
      const group = groupArea.querySelector(".type-group-header rotv-checkbox");
      const children = groupArea.querySelectorAll(".wrapper rotv-checkbox");
  ...
  static async #onHandleSubmit(event, form, formData) {
    ...
    if ( (value < (min || -Infinity)) || (value > (max || Infinity)) ) {
      ...
      throw new Error(game.i18n.format(`ROTV.CompendiumBrowser.Selection.Warning.${suffix}`, {
        max, min, value,
        document: game.i18n.localize(`ROTV.CompendiumBrowser.Selection.Warning.Document.${pr.select(max || min)}`)
      }));
    }

    /**
     * Hook event that calls when a compendium browser is submitted with selected items.
     * @function rotv.compendiumBrowserSelection
     * @memberof hookEvents
     ...
     */
    Hooks.callAll("rotv.compendiumBrowserSelection", this, this.#selected);
  }
  ...
  static async #onSetType(event, target) {
    ...
    if ( target.defaultValue ) {
      ...
      for ( const child of target.closest(".type-group").querySelectorAll("rotv-checkbox[value]") ) {
  ...
  static async fetch(documentClass, { types=new Set(), filters=[], index=true, indexFields=new Set(), sort=true }={}) {
    ...
    let documents = game.packs
      ...
      .filter(p => (p.metadata.type === documentClass.metadata.name)
        ...
        && (!types.size || !p.metadata.flags.rotv?.types || new Set(p.metadata.flags.rotv.types).intersects(types)))
      ...
      .map(async p => await Promise.all((await p.getIndex({ fields: Array.from(indexFields) })
        ...
        .then(index => game.rotv.moduleArt.apply(index)))
  ...
  static injectSidebarButton(html) {
    ...
    button.innerHTML = `
      ...
      ${game.i18n.localize("ROTV.CompendiumBrowser.Action.Open")}
    `;
