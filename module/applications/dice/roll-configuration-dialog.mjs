import ApplicationRotV from "../api/application.mjs";
 ...
 */
export default class RollConfigurationDialog extends ApplicationRotV {
  ...
  static DEFAULT_OPTIONS = {
    ...
    tag: "form",
    window: {
      title: "ROTV.RollConfiguration.Title",
  ...
  static PARTS = {
    formulas: {
      template: "systems/rotv/templates/dice/roll-formulas.hbs"
    },
    configuration: {
      template: "systems/rotv/templates/dice/roll-configuration.hbs"
    },
    buttons: {
      template: "systems/rotv/templates/dice/roll-buttons.hbs"
    }
  };
  ...
  _buildConfig(config, formData, index) {
     ...
     * @param {number} index                   Index of the roll within all rolls being prepared.
     */
    Hooks.callAll("rotv.buildRollConfig", this, config, formData, index);
  ...
  static async #handleFormSubmission(event, form, formData) {
    ...
    await this.close({ rotv: { submitted: true } });
  }
  ...
  /** @override */
  _onClose(options = {}) {
    if ( !options.rotv?.submitted ) this.#rolls = [];
  }
