...
import ItemSheetRotV from "./item-sheet.mjs";

export default class ContainerSheet extends ItemSheetRotV {
  ...
  /** @inheritdoc */
  get template() {
    return "systems/rotv/templates/items/container.hbs";
  }
  ...
  _onDrop(event) {
    ...
    if ( Hooks.call("rotv.dropItemSheetData", this.item, this, data) === false ) return;
   ...
   * @returns {Promise<ItemRotV[]>}          The created Item objects.
   */
  async _onDropFolder(event, data) {
    ...
    if ( recursiveWarning ) ui.notifications.warn("ROTV.ContainerRecursiveError", { localize: true });
    if ( !items.length ) return [];

    // Create any remaining items
    const toCreate = await ItemRotV.createWithContents(items, {
      container: this.item,
      transformAll: itemData => itemData.type === "spell" ? ItemRotV.createScrollFromSpell(itemData) : itemData
    });
    if ( this.item.folder ) toCreate.forEach(d => d.folder = this.item.folder.id);
    return ItemRotV.createDocuments(toCreate, {pack: this.item.pack, parent: this.item.parent, keepId: true});
  }
   ...
   * @param {DragEvent} event              The concluding DragEvent which contains the drop data.
   * @param {object} data                  The data transfer extracted from the event.
   * @returns {Promise<ItemRotV[]|boolean>}  The created Item objects or `false` if it couldn't be created.
   * @protected
   */
  async _onDropItem(event, data) {
    ...
    const parentContainers = await this.item.system.allContainers();
    if ( (this.item.uuid === item.uuid) || parentContainers.includes(item) ) {
      ui.notifications.error("ROTV.ContainerRecursiveError", { localize: true });
      return;
    }
    ...
    if ( this.item.folder ) toCreate.forEach(d => d.folder = this.item.folder.id);
    return ItemRotV.createDocuments(toCreate, {pack: this.item.pack, parent: this.item.actor, keepId: true});
  }
   ...
   * @param {ItemRotV} item      The item that needs to be sorted.
   * @protected
   */
