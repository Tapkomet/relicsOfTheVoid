 ...
 */
export default class ItemCompendiumRotV extends Compendium {
  ...
  async _handleDroppedEntry(target, data) {
    ...
    if ( !this._entryAlreadyExists(item) ) {
      ...
      if ( contents?.size ) {
        const toCreate = await ItemRotV.createWithContents([item], {transformAll: item => item.toCompendium(item)});
        const folder = target?.closest("[data-folder-id]")?.dataset.folderId;
        if ( folder ) toCreate.map(d => d.folder = folder);
        [item] = await ItemRotV.createDocuments(toCreate, {pack: this.collection.collection, keepId: true});
      }
    }
