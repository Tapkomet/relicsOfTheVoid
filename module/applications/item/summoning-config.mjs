...
export default class SummoningConfig extends DocumentSheet {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["rotv", "summoning-config"],
      dragDrop: [{ dropSelector: "form" }],
      template: "systems/rotv/templates/apps/summoning-config.hbs",
      width: 500,
      height: "auto",
      sheetConfig: false,
      closeOnSubmit: false,
  ...
  /** @inheritDoc */
  get title() {
    return `${game.i18n.localize("ROTV.Summoning.Configuration")}: ${this.document.name}`;
  }
  ...
  async getData(options={}) {
    ...
    context.profiles = this.profiles.map(p => {
      ...
      if ( this.document.system.summons.mode === "cr" ) {
        profile.creatureTypes = Object.entries(CONFIG.ROTV.creatureTypes).reduce((obj, [k, c]) => {
    ...
    context.creatureSizes = Object.entries(CONFIG.ROTV.actorSizes).reduce((obj, [k, c]) => {
      obj[k] = { label: c.label, selected: context.summons?.creatureSizes.has(k) ? "selected" : "" };
      return obj;
    }, {});
    context.creatureTypes = Object.entries(CONFIG.ROTV.creatureTypes).reduce((obj, [k, c]) => {
