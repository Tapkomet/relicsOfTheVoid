 ...
 *
 * @param {ItemRotV} item                                Item that is being used.
...
export default class AbilityUseDialog extends Dialog {
  constructor(item, dialogData={}, options={}) {
    ...
    this.options.classes = ["rotv", "dialog"];
  ...
  static async create(item, config, options={}) {
    ...
    const data = {
      ...
      title: game.i18n.format("ROTV.AbilityUseHint", {
    ...
    const html = await renderTemplate("systems/rotv/templates/apps/ability-use.hbs", data);

    // Create the Dialog and return data as a Promise
    const isSpell = item.type === "spell";
    const label = game.i18n.localize(`ROTV.AbilityUse${isSpell ? "Cast" : "Use"}`);
    return new Promise(resolve => {
      const dlg = new this(item, {
        title: `${item.name}: ${game.i18n.localize("ROTV.AbilityUseConfig")}`,
        content: html,
        buttons: {
          use: {
   ...
   * @param {ItemRotV} item     The item being used.
  ...
  static _createConcentrationOptions(item) {
    ...
    return effects.reduce((acc, effect) => {
      ...
      acc.push({
        name: effect.id,
        label: data?.name ?? item.actor.items.get(data)?.name ?? game.i18n.localize("ROTV.ConcentratingItemless")
      });
      return acc;
    }, []);
  }
   ...
   * @param {ActorRotV} actor  The actor with spell slots.
  ...
  static _createSpellSlotOptions(actor, level) {
    ...
    let lmax = 0;
    const options = Array.fromRange(Object.keys(CONFIG.ROTV.spellLevels).length).reduce((arr, i) => {
      if ( i < level ) return arr;
      const label = CONFIG.ROTV.spellLevels[i];
      ...
      arr.push({
        key: `spell${i}`,
        level: i,
        label: i > 0 ? game.i18n.format("ROTV.SpellLevelSlot", {level: label, n: slots}) : label,
        canCast: max > 0,
        hasSlots: slots > 0
      });
      return arr;
    ...
    for ( const k of Object.keys(CONFIG.ROTV.spellcastingTypes) ) {
      ...
      if ( spellData.level >= level ) {
        options.push({
          ...
          label: `${game.i18n.format(`ROTV.SpellLevel${k.capitalize()}`, {level: spellData.level, n: spellData.value})}`,
          canCast: true,
  ...
  static _createResourceOptions(item) {
    ...
    const spellLevels = Object.keys(CONFIG.ROTV.spellLevels).length - 1;
    ...
    switch ( consume.type ) {
      ...
      case "hitDice": {
        ...
        if ( ["smallest", "largest"].includes(consume.target) ) {
          label = game.i18n.localize(`ROTV.ConsumeHitDice${consume.target.capitalize()}Long`);
          value = target.system.attributes.hd.value;
        } else {
          value = item.actor.system.attributes.hd.bySize[consume.target] ?? 0;
          label = `${game.i18n.localize("ROTV.HitDice")} (${consume.target})`;
        }
        break;
      }
    }
  ...
  static _getAbilityUseNote(item, config) {
    ...
    // Zero quantity
    if ( quantity <= 0 ) return game.i18n.localize("ROTV.AbilityUseUnavailableHint");

    // Abilities which use Recharge
    if ( config.consumeUsage && recharge?.value ) {
      return game.i18n.format(recharge.charged ? "ROTV.AbilityUseChargedHint" : "ROTV.AbilityUseRechargeHint", {
        type: game.i18n.localize(CONFIG.Item.typeLabels[item.type])
      });
    }
    ...
    // Consumables
    if ( uses.autoDestroy ) {
      let str = "ROTV.AbilityUseNormalHint";
      if ( uses.value > 1 ) str = "ROTV.AbilityUseConsumableChargeHint";
      else if ( quantity > 1 ) str = "ROTV.AbilityUseConsumableQuantityHint";
      return game.i18n.format(str, {
        type: game.i18n.localize(`ROTV.Consumable${item.system.type.value.capitalize()}`),
        value: uses.value,
        quantity: quantity,
        max: uses.max,
        per: CONFIG.ROTV.limitedUsePeriods[uses.per]?.label
      });
    }

    // Other Items
    else {
      return game.i18n.format(`ROTV.AbilityUse${uses.value ? "Normal" : "Unavailable"}Hint`, {
        type: game.i18n.localize(CONFIG.Item.typeLabels[item.type]),
        value: uses.value,
        max: uses.max,
        per: CONFIG.ROTV.limitedUsePeriods[uses.per]?.label
  ...
  static _getAbilityUseWarnings(data, options={}) {
    ...
    if ( (scale === "slot") && data.slotOptions.every(o => !o.hasSlots) ) {
      ...
      warnings.push(game.i18n.format("ROTV.SpellCastNoSlotsLeft", {
        name: item.name
      }));
      ...
      warnings.push(game.i18n.format("ROTV.SpellCastNoSlots", {
        level: CONFIG.ROTV.spellLevels[level],
        name: item.name
      }));
    } else if ( (scale === "resource") && foundry.utils.isEmpty(data.resourceOptions) ) {
      // Warn that the resource does not have enough left.
      warnings.push(game.i18n.format("ROTV.ConsumeWarningNoQuantity", {
        name: item.name,
        type: CONFIG.ROTV.abilityConsumptionTypes[consume.type]
      }));
    }
    ...
    if ( item.hasResource ) {
      ...
      if ( isItem && !item.actor.items.get(consume.target) ) {
        warnings.push(game.i18n.format("ROTV.ConsumeWarningNoSource", {
          name: item.name, type: CONFIG.ROTV.abilityConsumptionTypes[consume.type]
        }));
      }
    }
    ...
    if ( item.type === "consumable" ) {
      const type = game.i18n.localize(`ROTV.Consumable${item.system.type.value.capitalize()}`);
      if ( this._willLowerQuantity(item) && (quantity === 1) ) {
        warnings.push(game.i18n.format("ROTV.AbilityUseConsumableDestroyHint", {type}));
      }
      ...
      if ( resource && (resource.system.quantity === 1) && this._willLowerQuantity(resource, qty) ) {
        warnings.push(game.i18n.format("ROTV.AbilityUseConsumableDestroyResourceHint", {type, name: resource.name}));
      }
    }
   ...
   * @param {ItemRotV} item       The item targeted for updates.
  ...
  _onChangeSlotLevel(event) {
    ...
    this._updateProfilesInput(
      "enchantmentProfile", "ROTV.Enchantment.Label", this.constructor._createEnchantmentOptions(item)
    );
    this._updateProfilesInput(
      "summonsProfile", "ROTV.Summoning.Profile.Label", this.constructor._createSummoningOptions(item)
    );
  }
