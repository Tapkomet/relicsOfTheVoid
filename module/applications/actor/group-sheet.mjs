...
export default class GroupActorSheet extends ActorSheetMixin(ActorSheet) {
  ...
  async getData(options={}) {
    ...
    // Text labels
    context.labels = {
      currencies: Object.entries(CONFIG.ROTV.currencies).reduce((obj, [k, c]) => {
        obj[k] = c.label;
        return obj;
      }, {})
    };
  ...
  #prepareMovementSpeed() {
    ...
    let speeds = [
      [movement.land, `${game.i18n.localize("ROTV.MovementLand")} ${movement.land}`],
      [movement.water, `${game.i18n.localize("ROTV.MovementWater")} ${movement.water}`],
      [movement.air, `${game.i18n.localize("ROTV.MovementAir")} ${movement.air}`]
    ];
