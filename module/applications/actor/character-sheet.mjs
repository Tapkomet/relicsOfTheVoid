import ActorSheetRotV from "./base-sheet.mjs";
 ...
 */
export default class ActorSheetRotVCharacter extends ActorSheetRotV {

  /** @inheritDoc */
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["rotv", "sheet", "actor", "character"]
    });
  }
  ...
  async getData(options={}) {
    ...
    context.resources = ["primary", "secondary", "tertiary"].reduce((arr, r) => {
      const res = foundry.utils.mergeObject(context.actor.system.resources[r] || {}, {
        name: r,
        placeholder: game.i18n.localize(`ROTV.Resource${r.titleCase()}`)
      }, {inplace: false});
    ...
    return foundry.utils.mergeObject(context, {
      disableExperience: game.settings.get("rotv", "disableExperienceTracking"),
      ...
      weightUnit: game.i18n.localize(`ROTV.Abbreviation${
        game.settings.get("rotv", "metricWeightUnits") ? "Kg" : "Lbs"}`),
  ...
  _prepareItems(context) {
    ...
    let {items, spells, feats, races, backgrounds, classes, subclasses} = context.items.reduce((obj, item) => {
      ...
      if ( item.system.attunement ) ctx.attunement = item.system.attuned ? {
        icon: "fa-sun",
        cls: "attuned",
        title: "ROTV.AttunementAttuned"
      } : {
        icon: "fa-sun",
        cls: "not-attuned",
        title: CONFIG.ROTV.attunementTypes[item.system.attunement]
      };
      ...
      // Item grouping
      ctx.ungroup = "passive";
      const [originId] = item.getFlag("rotv", "advancementOrigin")?.split(".") ?? [];
    ...
    const maxLevelDelta = CONFIG.ROTV.maxLevel - this.actor.system.details.level;
    classes = classes.reduce((arr, cls) => {
      const ctx = context.itemContext[cls.id] ??= {};
      ctx.availableLevels = Array.fromRange(CONFIG.ROTV.maxLevel + 1).slice(1).map(level => {
    ...
    for ( const subclass of subclasses ) {
      feats.push(subclass);
      const message = game.i18n.format("ROTV.SubclassMismatchWarn", {
    ...
    const features = {
      ...
      active: {
        label: "ROTV.FeatureActive", items: [],
        hasActions: true, dataset: {type: "feat", "activation.type": "action"} },
      passive: {
        label: "ROTV.FeaturePassive", items: [],
   ...
   * @param {ItemRotV} item     Item being prepared for display.
  ...
  _prepareItem(item, context) {
    if ( item.type === "spell" ) {
      ...
      if ( isAlways ) context.toggleTitle = CONFIG.ROTV.spellPreparationModes.always.label;
      else if ( isPrepared ) context.toggleTitle = CONFIG.ROTV.spellPreparationModes.prepared.label;
      else context.toggleTitle = game.i18n.localize("ROTV.SpellUnprepared");
    }
    else {
      const isActive = !!item.system.equipped;
      context.toggleClass = isActive ? "active" : "";
      context.toggleTitle = game.i18n.localize(isActive ? "ROTV.Equipped" : "ROTV.Unequipped");
  ...
  async _onDropSingleItem(itemData) {
    ...
    if ( itemData.type === "class" ) {
      ...
      itemData.system.levels = Math.min(itemData.system.levels, CONFIG.ROTV.maxLevel - charLevel);
    ...
    // If a subclass is dropped, ensure it doesn't match another subclass with the same identifier
      ...
      if ( other ) {
        const err = game.i18n.format("ROTV.SubclassDuplicateError", {identifier: other.identifier});
        ui.notifications.error(err);
        return false;
      }
      const cls = this.actor.itemTypes.class.find(i => i.identifier === itemData.system.classIdentifier);
      if ( cls && cls.subclass ) {
        const err = game.i18n.format("ROTV.SubclassAssignmentError", {class: cls.name, subclass: cls.subclass.name});
        ui.notifications.error(err);
        return false;
      }
    }
