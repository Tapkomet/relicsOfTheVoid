...
export default class ActorSheetFlags extends BaseConfigSheet {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      id: "actor-flags",
      classes: ["rotv"],
      template: "systems/rotv/templates/apps/actor-flags.hbs",
      width: 500,
      closeOnSubmit: true
    });
  }
  ...
  /** @inheritDoc */
  get title() {
    return `${game.i18n.localize("ROTV.FlagsTitle")}: ${this.object.name}`;
  }
  ...
  _getFlags() {
    ...
    for ( let [k, v] of Object.entries(CONFIG.ROTV.characterFlags) ) {
      if ( !flags.hasOwnProperty(v.section) ) flags[v.section] = {};
      let flag = foundry.utils.deepClone(v);
      flag.type = v.type.name;
      flag.isCheckbox = v.type === Boolean;
      flag.isSelect = v.hasOwnProperty("choices");
      flag.value = foundry.utils.getProperty(baseData.flags, `rotv.${k}`);
      flags[v.section][`flags.rotv.${k}`] = flag;
    }
    return flags;
  }
  ...
  _getBonuses() {
    ...
    const bonuses = [
      {name: "system.bonuses.mwak.attack", label: "ROTV.BonusMWAttack"},
      ...
      {name: "system.bonuses.spell.dc", label: "ROTV.BonusSpellDC"}
    ];
  ...
  async _updateObject(event, formData) {
    ...
    const flags = updateData.flags.rotv;
    for ( let [k, v] of Object.entries(flags) ) {
      if ( [undefined, null, "", false, 0].includes(v) ) {
        delete flags[k];
        if ( foundry.utils.hasProperty(src.flags, `rotv.${k}`) ) flags[`-=${k}`] = null;
      }
    }
