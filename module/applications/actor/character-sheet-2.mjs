...
import ActorSheetRotVCharacter from "./character-sheet.mjs";
 ...
 * @mixes ActorSheetV2
 */
export default class ActorSheetRotVCharacter2 extends ActorSheetV2Mixin(ActorSheetRotVCharacter) {
  /** @inheritDoc */
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["rotv2", "sheet", "actor", "character", "vertical-tabs"],
  ...
  /** @override */
  static TABS = [
    { tab: "details", label: "ROTV.Details", icon: "fas fa-cog" },
  ...
  get template() {
    ...
    return "systems/rotv/templates/actors/character-sheet-2.hbs";
  }
  ...
  async getData(options) {
    ...
    context.exhaustion = Array.fromRange(max, 1).reduce((acc, n) => {
      const label = game.i18n.format("ROTV.ExhaustionLevel", { n });
      const classes = ["pip"];
    ...
    context.speed = Object.entries(CONFIG.ROTV.movementTypes).reduce((obj, [k, label]) => {
      ...
      return obj;
    }, { value: 0, label: CONFIG.ROTV.movementTypes.walk });
    ...
    context.abilityRows.optional = Object.keys(CONFIG.ROTV.abilities).length - 6;
    ...
    for ( let ability of Object.values(context.abilities) ) {
      ...
      ability.hover = CONFIG.ROTV.proficiencyLevels[ability.proficient];
    }
    ...
    if ( this.actor.statuses.has(CONFIG.specialStatusEffects.CONCENTRATING) || context.editable ) {
      context.saves.concentration = {
        ...
        label: game.i18n.localize("ROTV.Concentration"),
        abbr: game.i18n.localize("ROTV.Concentration"),
    ...
    // Size
    context.size = {
      label: CONFIG.ROTV.actorSizes[traits.size]?.label ?? traits.size,
      abbr: CONFIG.ROTV.actorSizes[traits.size]?.abbreviation ?? "—",
    ...
    for ( const [key, entry] of Object.entries(context.skills).concat(Object.entries(context.tools)) ) {
      ...
      if ( key in CONFIG.ROTV.skills ) entry.reference = CONFIG.ROTV.skills[key].reference;
      else if ( key in CONFIG.ROTV.toolIds ) entry.reference = Trait.getBaseItemUUID(CONFIG.ROTV.toolIds[key]);
    }

    // Character Background
    context.creatureType = {
      class: details.type.value === "custom" ? "none" : "",
      icon: CONFIG.ROTV.creatureTypes[details.type.value]?.icon ?? "icons/svg/mystery-man.svg",
      title: details.type.value === "custom"
        ? details.type.custom
        : CONFIG.ROTV.creatureTypes[details.type.value]?.label,
      reference: CONFIG.ROTV.creatureTypes[details.type.value]?.reference,
      subtitle: details.type.subtype
    };

    if ( details.race instanceof rotv.documents.ItemRotV ) context.race = details.race;
    if ( details.background instanceof rotv.documents.ItemRotV ) context.background = details.background;
    ...
    for ( const item of Object.values(this.actor.classes).sort((a, b) => b.system.levels - a.system.levels) ) {
      ...
      context.spellcasting.push({
        label: game.i18n.format("ROTV.SpellcastingClass", { class: name }),
  ...
  _prepareItems(context) {
    ...
    Object.values(this.actor.classes ?? {}).sort((a, b) => b.system.levels - a.system.levels).forEach(cls => {
      features.push({
        label: game.i18n.format("ROTV.FeaturesClass", { class: cls.name }),
        items: [],
    ...
    if ( this.actor.system.details.background instanceof rotv.documents.ItemRotV ) {
      features.push({ label: "ROTV.FeaturesBackground", items: [], dataset: { type: "background" } });
    }

    features.push({ label: "ROTV.FeaturesOther", items: [], dataset: { type: "other" } });
    ...
    context.features = {
      ...
      filters: [
        ...
        { key: "lr", label: "ROTV.LongRest" },
        { key: "concentration", label: "ROTV.Concentration" },
        { key: "mgc", label: "ROTV.Item.Property.Magical" }
      ]
    };
    ...
    features.forEach(section => {
      section.categories = [
        ...
        { classes: "item-recovery", label: "ROTV.Recovery", partial: "rotv.column-recovery" },
  ...
  activateListeners(html) {
    ...
    if ( featuresElement ) new ContextMenuRotV(html, ".pills-lg [data-item-id]", [], {
  ...
  _onDragStart(event) {
    ...
    let type;
    if ( key in CONFIG.ROTV.skills ) type = "skill";
    else if ( key in CONFIG.ROTV.toolIds ) type = "tool";
    else if ( modes[preparationMode]?.upcast && (level !== "0") && isSlots ) type = "slots";
    if ( !type ) return super._onDragStart(event);
    const dragData = { rotv: { action: "favorite", type } };
    if ( type === "slots" ) dragData.rotv.id = (preparationMode === "prepared") ? `spell${level}` : preparationMode;
    else dragData.rotv.id = key;
  ...
  _toggleDeathTray(open) {
    ...
    tab.dataset.tooltip = `ROTV.DeathSave${this._deathTrayOpen ? "Hide" : "Show"}`;
  ...
  _onConfigureSheet(event) {
    ...
    new SheetConfigRotV(this.document, {
  ...
  async _onDrop(event) {
    ...
    const { action, type, id } = data.rotv ?? {};
   ...
   * @param {PointerEvent} event         The triggering event.
   * @param {ActorFavoritesRotV} favorite  The favorite that was dropped.
  ...
  async _onUseFavorite(event) {
    ...
    if ( favorite instanceof rotv.documents.ItemRotV ) return favorite.use({}, { event });
    if ( favorite instanceof rotv.documents.ActiveEffectRotV ) return favorite.update({ disabled: !favorite.disabled });
  }
  ...
  async _prepareFavorites() {
    ...
    const resources = Object.entries(this.actor.system.resources).reduce((arr, [k, r]) => {
      ...
      if ( label && max ) arr.push({
        ...
        title: label,
        subtitle: [
          sr ? game.i18n.localize("ROTV.AbbreviationSR") : null,
          lr ? game.i18n.localize("ROTV.AbbreviationLR") : null
        ].filterJoin(" &bull; ")
      });
      return arr;
    }, []);
    ...
    return resources.concat(await this.actor.system.favorites.reduce(async (arr, f) => {
      ...
      if ( suppressed ) subtitle = game.i18n.localize("ROTV.Suppressed");
      arr.push({
   ...
   * @returns {Promise<FavoriteDataRotV|void>}
   * @protected
   */
  async _getFavoriteData(type, id) {
    ...
    if ( type === "slots" ) {
      ...
      if ( !/spell\d+/.test(id) ) return {
        uses, level,
        title: game.i18n.localize(`ROTV.SpellSlots${id.capitalize()}`),
        subtitle: [
          game.i18n.localize(`ROTV.SpellLevel${level}`),
          game.i18n.localize(`ROTV.Abbreviation${CONFIG.ROTV.spellcastingTypes[id]?.shortRest ? "SR" : "LR"}`)
        ],
        img: CONFIG.ROTV.spellcastingTypes[id]?.img || CONFIG.ROTV.spellcastingTypes.pact.img
      };

      const plurals = new Intl.PluralRules(game.i18n.lang, { type: "ordinal" });
      const isSR = CONFIG.ROTV.spellcastingTypes.leveled.shortRest;
      return {
        uses, level,
        title: game.i18n.format(`ROTV.SpellSlotsN.${plurals.select(level)}`, { n: level }),
        subtitle: game.i18n.localize(`ROTV.Abbreviation${isSR ? "SR" : "LR"}`),
        img: CONFIG.ROTV.spellcastingTypes.leveled.img.replace("{id}", id)
      };
    }
    ...
    // Skills & Tools
      ...
      const subtitle = game.i18n.format("ROTV.AbilityPromptTitle", {
        ability: CONFIG.ROTV.abilities[ability].label
      });
      let img;
      let title;
      let reference;
      if ( type === "tool" ) {
        reference = Trait.getBaseItemUUID(CONFIG.ROTV.toolIds[id]);
