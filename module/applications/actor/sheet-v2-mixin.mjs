...
import TabsRotV from "../tabs.mjs";

/**
 * Adds common V2 sheet functionality.
 * @param {typeof ActorSheetRotV} Base  The base class being mixed.
...
export default function ActorSheetV2Mixin(Base) {
  return class ActorSheetV2 extends Base {
    constructor(object, options={}) {
      ...
      const { width, height } = game.user.getFlag("rotv", `sheetPrefs.${key}`) ?? {};
    ...
    /**
     * @typedef {object} SheetTabDescriptorRotV
    ...
    async _renderOuter() {
      ...
      if ( this.isEditable ) {
        ...
        toggle.dataset.tooltip = "ROTV.SheetModeEdit";
        toggle.setAttribute("aria-label", game.i18n.localize("ROTV.SheetModeEdit"));
      ...
      nav.append(...this.constructor.TABS.map(({ tab, label, icon, svg }) => {
        ...
        else if ( svg ) item.innerHTML = `<rotv-icon src="systems/rotv/icons/svg/${svg}.svg"></rotv-icon>`;
        return item;
      }));
    ...
    async getData(options) {
      ...
      context.sidebarCollapsed = !!game.user.getFlag("rotv", `${sheetPrefs}.collapseSidebar`);
      if ( context.sidebarCollapsed ) context.cssClass += " collapsed";
      const { attributes } = this.actor.system;

      // Portrait
      const showTokenPortrait = this.actor.getFlag("rotv", "showTokenPortrait") === true;
      ...
      ["success", "failure"].forEach(deathSave => {
        ...
        for ( let i = 1; i < 4; i++ ) {
          ...
          const i18nKey = `ROTV.DeathSave${deathSave.titleCase()}Label`;
      ...
      // Senses
      context.senses = Object.entries(CONFIG.ROTV.senses).reduce((obj, [k, label]) => {
      ...
      context.conditions = Object.entries(CONFIG.ROTV.conditionTypes).reduce((arr, [k, c]) => {
        if ( c.pseudo ) return arr; // Filter out pseudo-conditions.
        const { label: name, icon, reference } = c;
        const id = staticID(`rotv${k}`);
        conditionIds.add(id);
      ...
      for ( const category of Object.values(context.effects) ) {
        category.effects = await category.effects.reduce(async (arr, effect) => {
          ...
          if ( (source instanceof rotv.documents.ActiveEffectRotV) && (source.target !== this.object) ) {
          ...
          arr.push({
            ...
            hasTooltip: source instanceof rotv.documents.ItemRotV
          });
          return arr;
        }, []);
      }
    ...
    _prepareTraits() {
      const traits = {};
      for ( const [trait, config] of Object.entries(CONFIG.ROTV.traits) ) {
        ...
        values = values.map(key => {
          ...
          if ( data.bypasses?.size && CONFIG.ROTV.damageTypes[key]?.isPhysical ) icons.push(...data.bypasses.map(p => {
            const type = CONFIG.ROTV.itemProperties[p]?.label;
            return { icon: p, label: game.i18n.format("ROTV.DamagePhysicalBypassesShort", { type }) };
          }));
          return value;
        });
      ...
      if ( this.document.hasConditionEffect("petrification") ) {
        traits.dr = [{ label: game.i18n.localize("ROTV.DamageAll") }];
      }
      ...
      if ( dm ) {
        ...
        const values = Object.entries(dm.amount).map(([k, v]) => {
          ...
          const value = {
            label: `${CONFIG.ROTV.damageTypes[k]?.label ?? k} ${formatNumber(total, { signDisplay: "always" })}`,
            color: total > 0 ? "maroon" : "green"
          };
          const icons = value.icons = [];
          if ( dm.bypasses.size && CONFIG.ROTV.damageTypes[k]?.isPhysical ) icons.push(...dm.bypasses.map(p => {
            const type = CONFIG.ROTV.itemProperties[p]?.label;
            return { icon: p, label: game.i18n.format("ROTV.DamagePhysicalBypassesShort", { type }) };
          }));
          return value;
    ...
    _prepareItems(context) {
      ...
      context.spellbook.forEach(section => {
        ...
        section.pips = Array.fromRange(max, 1).map(n => {
          ...
          const label = filled
            ? game.i18n.format(`ROTV.SpellSlotN.${plurals.select(n)}`, { n })
            : game.i18n.localize("ROTV.SpellSlotExpended");
          const classes = ["pip"];
          if ( filled ) classes.push("filled");
    ...
    _prepareItem(item, ctx) {
      ...
      if ( item.type === "spell" ) {
        ...
        const abbr = {
          ...
          hour: "ROTV.TimeHourAbbr",
          day: "ROTV.TimeDayAbbr"
        ...
        if ( units && (units !== "none") ) {
          if ( units in CONFIG.ROTV.movementUnits ) {
            ctx.range = {
              distance: true,
              value: system.range.value,
              unit: game.i18n.localize(`ROTV.Dist${units.capitalize()}Abbr`)
            };
          }
        ...
        if ( config.prepares ) {
          ...
          ctx.preparation = {
            ...
            title: isAlways
              ? CONFIG.ROTV.spellPreparationModes.always.label
              : prepared
                ? CONFIG.ROTV.spellPreparationModes.prepared.label
                : game.i18n.localize("ROTV.SpellUnprepared")
          };
        }
      ...
      // Gear
        ...
        if ( "equipped" in system ) {
          ctx.equip = {
            ...
            title: `ROTV.${system.equipped ? "Equipped" : "Unequipped"}`,
    ...
    _getLabels() {
      ...
      labels.damageAndHealing = { ...CONFIG.ROTV.damageTypes, ...CONFIG.ROTV.healingTypes };
      return labels;
    }
    ...
    async _onChangeSheetMode(event) {
      ...
      const label = game.i18n.localize(`ROTV.SheetMode${toggle.checked ? "Play" : "Edit"}`);
    ...
    _onChangeTab(event, tabs, active) {
      ...
      const sidebarCollapsed = game.user.getFlag("rotv", `${sheetPrefs}.collapseSidebar`);
     ...
     * @returns {ActiveEffectRotV|ItemRotV|void}
     * @protected
     */
    _onCreateChild() {
      ...
      if ( activeTab === "effects" ) return ActiveEffect.implementation.create({
        name: game.i18n.localize("ROTV.EffectNew"),
    ...
    _onResize(event) {
      ...
      game.user.setFlag("rotv", `sheetPrefs.${key}`, { width, height });
    }
    ...
    async _onToggleDescription(event) {
      ...
      if ( expanded ) {
        ...
        summary.innerHTML = await renderTemplate("systems/rotv/templates/items/parts/item-summary.hbs", context);
    ...
    _onToggleSidebar() {
      ...
      game.user.setFlag("rotv", `sheetPrefs.${this.actor.type}.tabs.${activeTab}.collapseSidebar`, collapsed);
    }
    ...
    _applyItemTooltips(element) {
      ...
      `;
      element.dataset.tooltipClass = "rotv2 rotv-tooltip item-tooltip";
     ...
     * @param {ItemRotV} item  Item on the sheet.
     * @returns {boolean}
     */
    canExpand(item) {
