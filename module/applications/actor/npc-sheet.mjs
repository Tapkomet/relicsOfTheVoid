 ...
 */
export default class ActorSheetRotVNPC extends ActorSheetRotV {

  /** @inheritDoc */
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["rotv", "sheet", "actor", "npc"],
      width: 600
    });
  }
  ...
  _prepareItems(context) {
    ...
    const features = {
      weapons: { label: game.i18n.localize("ROTV.AttackPl"), items: [], hasActions: true,
        dataset: {type: "weapon", "weapon-type": "natural"} },
      actions: { label: game.i18n.localize("ROTV.ActionPl"), items: [], hasActions: true,
        dataset: {type: "feat", "activation.type": "action"} },
      passive: { label: game.i18n.localize("ROTV.Features"), items: [], dataset: {type: "feat"} },
      equipment: { label: game.i18n.localize("ROTV.Inventory"), items: [], dataset: {type: "loot"}}
    };
    ...
    let [spells, other] = context.items.reduce((arr, item) => {
      ...
      if ( item.type === "class" ) ctx.availableLevels = Array.fromRange(CONFIG.ROTV.maxLevel, 1).map(level => ({
  ...
  getArmorLabel() {
    ...
    const label = [];
    if ( ac.calc === "default" ) label.push(this.actor.armor?.name || game.i18n.localize("ROTV.ArmorClassUnarmored"));
    else label.push(game.i18n.localize(CONFIG.ROTV.armorClasses[ac.calc].label));
