...
export default class Award extends DialogMixin(FormApplication) {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["rotv2", "award", "dialog"],
      template: "systems/rotv/templates/apps/award.hbs",
      title: "ROTV.Award.Title",
      width: 350,
      height: "auto",
      currency: null,
      xp: null,
  ...
  getData(options={}) {
    ...
    context.currency = Object.entries(CONFIG.ROTV.currencies).reduce((obj, [k, { label }]) => {
    ...
    context.hideXP = game.settings.get("rotv", "disableExperienceTracking");
    context.noPrimaryParty = !game.settings.get("rotv", "primaryParty")?.actor && !this.isPartyAward;
  ...
  static prepareDestinations(destinations, savedDestinations) {
    const icons = {
      container: '<rotv-icon class="fa-fw" src="systems/rotv/icons/svg/backpack.svg"></rotv-icon>',
   ...
   * @param {Set<ActorRotV>} destinations  Selected destinations to save.
   * @protected
   */
  _saveDestinations(destinations) {
    const target = this.isPartyAward ? this.object : game.user;
    target.setFlag("rotv", "awardDestinations", destinations);
  }
   ...
   * @param {(ActorRotV|ItemRotV)[]} destinations  Documents that should receive the currency.
   * @param {object} [config={}]
   * @param {boolean} [config.each=false]      Award the specified amount to each player, rather than splitting it.
   * @param {ActorRotV|ItemRotV} [config.origin]   Document from which to move the currency, if not a freeform award.
   * @param {Map<ActorRotV|ItemRotV, object>} [config.results]  Results of the award operation.
   */
   ...
   * @param {ActorRotV[]} destinations   Actors that should receive the XP.
   * @param {object} [config={}]
   * @param {boolean} [config.each=false]      Award the specified amount to each player, rather than splitting it.
   * @param {ActorRotV} [config.origin]  Group actor from which to transfer the XP.
   * @param {Map<ActorRotV|ItemRotV, object>} [config.results]  Results of the award operation.
   */
   ...
   * @param {Map<ActorRotV|ItemRotV, object>} results  Results of any award operations.
   */
  static async displayAwardMessages(results) {
    ...
    for ( const [destination, result] of results ) {
      ...
      if ( result.xp ) entries.push(`
        <span class="award-entry">
          ${formatNumber(result.xp)} ${game.i18n.localize("ROTV.ExperiencePointsAbbr")}
        </span>
      `);
      if ( !entries.length ) continue;

      const content = game.i18n.format("ROTV.Award.Message", {
        name: destination.name, award: `<span class="rotv2">${game.i18n.getListFormatter().format(entries)}</span>`
      });
  ...
  static async handleAward(message) {
    if ( !game.user.isGM ) {
      ui.notifications.error("ROTV.Award.NotGMError", { localize: true });
      return;
    }
    ...
    try {
      ...
      // Otherwise show the UI with defaults
        ...
        const savedDestinations = game.user.getFlag("rotv", "awardDestinations");
  ...
  static parseAwardCommand(message) {
    ...
    for ( const part of command.split(" ") ) {
      ...
      try {
        new Roll(amount);
        if ( label in CONFIG.ROTV.currencies ) currency[label] = amount;
    ...
    if ( unrecognized.length ) throw new Error(game.i18n.format("ROTV.Award.UnrecognizedWarning", {
