 ...
 */
export default class TooltipsRotV {
  /* -------------------------------------------- */
  /*  Properties & Getters                        */
  /* -------------------------------------------- */
  ...
  async _onTooltipActivate() {
    ...
    if ( loading?.dataset.uuid ) {
      ...
      if ( doc instanceof rotv.documents.ActorRotV ) return this._onHoverActor(doc);
      return this._onHoverContentLink(doc);
    }
   ...
   * @param {ActorRotV} actor  The actor.
   * @protected
   */
  ...
  async _onHoverPassive(skill, ability, dc) {
    const skillConfig = CONFIG.ROTV.skills[skill];
    const abilityConfig = CONFIG.ROTV.abilities[ability ?? skillConfig.ability];

    let label;
    if ( skillConfig ) {
      label = game.i18n.format("ROTV.SkillPassiveSpecificHint", { skill: skillConfig.label, ability: abilityConfig.label });
    } else {
      ...
      label = game.i18n.format("ROTV.SkillPassiveHint", { skill: abilityConfig.label });
    }

    const party = game.settings.get("rotv", "primaryParty")?.actor;
    if ( !party ) {
    ...
    this.tooltip.classList.add("rotv-tooltip", "passive-tooltip");
    this.tooltip.innerHTML = await renderTemplate("systems/rotv/templates/journal/passive-tooltip.hbs", context);
