 ...
 */
export default class ActiveEffectRotV extends ActiveEffect {
  /**
   * Static ActiveEffect ID for various conditions.
   * @type {Record<string, string>}
   */
  static ID = {
    ENCUMBERED: staticID("rotvencumbered"),
    EXHAUSTION: staticID("rotvexhaustion")
  };
  ...
  get isAppliedEnchantment() {
    return (this.getFlag("rotv", "type") === "enchantment")
   ...
   * @returns {Promise<ActorRotV|ItemRotV|null>}
   */
  async getSource() {
    if ( (this.target instanceof rotv.documents.ActorRotV) && (this.parent instanceof rotv.documents.ItemRotV) ) {
      return this.parent;
    }
  ...
  /** @inheritdoc */
  apply(actor, change) {
    if ( change.key.startsWith("flags.rotv.") ) change = this._prepareFlagChange(actor, change);
    ...
    const getTargetType = field => {
      if ( (field instanceof FormulaField) || ActiveEffectRotV.FORMULA_FIELDS.has(change.key) ) return "formula";
   ...
   * @param {ActorRotV} actor                 The Actor to whom this effect should be applied
  ...
  _prepareFlagChange(actor, change) {
    ...
    const data = CONFIG.ROTV.characterFlags[key.replace("flags.rotv.", "")];
  ...
  determineSuppression() {
    ...
    if ( this.getFlag("rotv", "type") === "enchantment" ) return;
    if ( this.parent instanceof rotv.documents.ItemRotV ) this.isSuppressed = this.parent.areEffectsSuppressed;
  }
  ...
  _prepareExhaustionLevel() {
    const config = CONFIG.ROTV.conditionTypes.exhaustion;
    let level = this.getFlag("rotv", "exhaustionLevel");
    if ( !Number.isFinite(level) ) level = 1;
    this.img = this.constructor._getExhaustionImage(level);
    this.name = `${game.i18n.localize("ROTV.Exhaustion")} ${level}`;
    if ( level >= config.levels ) {
      this.statuses.add("dead");
      CONFIG.ROTV.statusEffects.dead.statuses?.forEach(s => this.statuses.add(s));
    }
  }
   ...
   * @returns {Promise<FavoriteDataRotV>}
   */
   ...
   * @returns {Promise<ActiveEffectRotV[]|void>}      Created rider effects.
   */
  async createRiderConditions() {
    ...
    const createRider = async id => {
      const existing = this.parent.effects.get(staticID(`rotv${id}`));
      if ( existing ) return;
  ...
  async createRiderEnchantments(options) {
    ...
    // Create Effects
    const riderEffects = (this.getFlag("rotv", "enchantment.riders.effect") ?? []).map(id => {
      const effectData = origin.effects.get(id)?.toObject();
      if ( effectData ) {
        delete effectData._id;
        delete effectData.flags?.rotv?.rider;
    ...
    if ( this.parent.isEmbedded ) {
      const riderItems = await Promise.all((this.getFlag("rotv", "enchantment.riders.item") ?? []).map(async uuid => {
        const itemData = (await fromUuid(uuid))?.toObject();
        if ( itemData ) {
          delete itemData._id;
          foundry.utils.setProperty(itemData, "flags.rotv.enchantment", { origin: this.uuid });
        }
        return itemData;
      }));
  ...
  async _preCreate(data, options, user) {
    ...
    if ( (this.getFlag("rotv", "type") === "enchantment") && (this.parent instanceof Actor) ) {
      ui.notifications.error("ROTV.Enchantment.Warning.NotOnActor", { localize: true });
      return false;
    }
  ...
  _onUpdate(data, options, userId) {
    ...
    const originalLevel = foundry.utils.getProperty(options, "rotv.originalExhaustion");
    const newLevel = foundry.utils.getProperty(data, "flags.rotv.exhaustionLevel");
    const originalEncumbrance = foundry.utils.getProperty(options, "rotv.originalEncumbrance");
    ...
    // Display proper scrolling status effects for encumbrance
      ...
      if ( !increase ) this.name = CONFIG.ROTV.encumbrance.effects[originalEncumbrance].name;
  ...
  async _preDelete(options, user) {
    ...
    if ( dependents.length && !game.users.activeGM ) {
      ui.notifications.warn("ROTV.ConcentrationBreakWarning", { localize: true });
      return false;
    }
   ...
   * @param {ItemRotV} item       The item on which to begin concentrating.
  ...
  static createConcentrationEffectData(item, data={}) {
    ...
    const effectData = foundry.utils.mergeObject({
      ...statusEffect,
      name: `${game.i18n.localize("EFFECT.ROTV.StatusConcentrating")}: ${item.name}`,
      description: game.i18n.format("ROTV.ConcentratingOn", {
        name: item.name,
        type: game.i18n.localize(`TYPES.Item.${item.type}`)
      }),
      duration: ActiveEffectRotV.getEffectDurationFromItem(item),
      "flags.rotv.itemData": item.actor.items.has(item.id) ? item.id : item.toObject(),
      origin: item.uuid,
      statuses: [statusEffect.id].concat(statusEffect.statuses ?? [])
    }, data, {inplace: false});
    delete effectData.id;
    if ( item.type === "spell" ) effectData["flags.rotv.spellLevel"] = item.system.level;
  ...
  static onTokenHUDRender(app, html) {
    ...
    if ( Number.isFinite(level) && (level > 0) ) {
      const img = ActiveEffectRotV._getExhaustionImage(level);
  ...
  static _getExhaustionImage(level) {
    const split = CONFIG.ROTV.conditionTypes.exhaustion.icon.split(".");
    const ext = split.pop();
   ...
   * @param {ItemRotV} item     An item with a duration.
  ...
  static onClickTokenHUD(event) {
    ...
    if ( id === "exhaustion" ) ActiveEffectRotV._manageExhaustion(event, actor);
    else if ( id === "concentrating" ) ActiveEffectRotV._manageConcentration(event, actor);
  }
   ...
   * @param {PointerEvent} event        The triggering event.
   * @param {ActorRotV} actor             The actor belonging to the token.
   */
  static _manageExhaustion(event, actor) {
    ...
    const max = CONFIG.ROTV.conditionTypes.exhaustion.levels;
  ...
  static _manageConcentration(event, actor) {
    ...
    const choices = effects.reduce((acc, effect) => {
      ...
      acc[effect.id] = data?.name ?? actor.items.get(data)?.name ?? game.i18n.localize("ROTV.ConcentratingItemless");
      return acc;
    }, {});
    const options = HandlebarsHelpers.selectOptions(choices, { hash: { sort: true } });
    const content = `
    <form class="rotv">
      <p>${game.i18n.localize("ROTV.ConcentratingEndChoice")}</p>
      <div class="form-group">
        <label>${game.i18n.localize("ROTV.Source")}</label>
    ...
    Dialog.prompt({
      ...
      rejectClose: false,
      title: game.i18n.localize("ROTV.Concentration"),
      label: game.i18n.localize("ROTV.Confirm")
    });
  }
  ...
  addDependent(...dependent) {
    const dependents = this.getFlag("rotv", "dependents") ?? [];
    dependents.push(...dependent.map(d => ({ uuid: d.uuid })));
    return this.setFlag("rotv", "dependents", dependents);
  }
   ...
   * @returns {Array<ActiveEffectRotV|ItemRotV>}
   */
  getDependents() {
    return (this.getFlag("rotv", "dependents") || []).reduce((arr, { uuid }) => {
   ...
   * @param {ActorRotV|ItemRotV} doc  Document from which to determine the overrides.
  ...
  async richTooltip(enrichmentOptions={}) {
    const properties = [];
    if ( this.isSuppressed ) properties.push("ROTV.EffectType.Unavailable");
    else if ( this.disabled ) properties.push("ROTV.EffectType.Inactive");
    else if ( this.isTemporary ) properties.push("ROTV.EffectType.Temporary");
    else properties.push("ROTV.EffectType.Passive");
    if ( this.getFlag("rotv", "type") === "enchantment" ) properties.push("ROTV.Enchantment.Label");

    return {
      content: await renderTemplate(
        "systems/rotv/templates/effects/parts/effect-tooltip.hbs", {
          effect: this,
      ...
      classes: ["rotv2", "rotv-tooltip", "effect-tooltip"]
    };
  }
}
