...
export default class ItemChoiceAdvancement extends ItemGrantAdvancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      order: 50,
      icon: "systems/rotv/icons/svg/item-choice.svg",
      title: game.i18n.localize("ROTV.AdvancementItemChoiceTitle"),
      hint: game.i18n.localize("ROTV.AdvancementItemChoiceHint"),
      multiLevel: true,
      apps: {
  ...
  titleForLevel(level, { configMode=false }={}) {
    ...
    let tag;
    if ( data.count ) tag = game.i18n.format("ROTV.AdvancementItemChoiceChoose", { count: data.count });
    else if ( data.replacement ) tag = game.i18n.localize("ROTV.AdvancementItemChoiceReplacementTitle");
  ...
  summaryForLevel(level, { configMode=false }={}) {
    ...
    return Object.values(items).reduce((html, uuid) => html + game.rotv.utils.linkForUuid(uuid), "");
  }
  ...
  restore(level, data) {
    ...
    if ( data.replaced ) {
      if ( !original ) {
        throw new ItemChoiceAdvancement.ERROR(game.i18n.localize("ROTV.AdvancementItemChoiceNoOriginalError"));
      }
   ...
   * @param {ItemRotV} item                   Item that needs to be tested.
   * @param {object} config
  ...
  _validateItemType(item, { type, restriction, strict=true }={}) {
    ...
    if ( type && (type !== item.type) ) {
      ...
      if ( strict ) throw new Error(game.i18n.format("ROTV.AdvancementItemChoiceTypeWarning", {type: typeLabel}));
      return false;
    }

    // If additional type restrictions applied, make sure they are valid
    if ( (type === "feat") && restriction.type ) {
      const typeConfig = CONFIG.ROTV.featureTypes[restriction.type];
      ...
      if ( errorLabel ) {
        if ( strict ) throw new Error(game.i18n.format("ROTV.AdvancementItemChoiceTypeWarning", {type: errorLabel}));
        return false;
      }
    }
    ...
    if ( (type === "spell") && !Number.isNaN(l) && (item.system.level !== l) ) {
      const level = CONFIG.ROTV.spellLevels[l];
      if ( strict ) throw new Error(game.i18n.format("ROTV.AdvancementItemChoiceSpellLevelSpecificWarning", {level}));
      return false;
    }
