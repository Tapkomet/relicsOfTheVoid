...
export default class SizeAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      order: 25,
      icon: "systems/rotv/icons/svg/size.svg",
      title: game.i18n.localize("ROTV.AdvancementSizeTitle"),
      hint: game.i18n.localize("ROTV.AdvancementSizeHint"),
      apps: {
        config: SizeConfig,
        flow: SizeFlow
      }
  ...
  get automaticHint() {
    ...
    if ( this.configuration.sizes.size === 1 ) return game.i18n.format("ROTV.AdvancementSizeFlowHintSingle", {
      size: CONFIG.ROTV.actorSizes[this.configuration.sizes.first()].label
    });

    const listFormatter = new Intl.ListFormat(game.i18n.lang, { type: "disjunction" });
    return game.i18n.format("ROTV.AdvancementSizeflowHintMultiple", {
      sizes: listFormatter.format(this.configuration.sizes.map(s => CONFIG.ROTV.actorSizes[s].label))
    });
  }
  ...
  summaryForLevel(level, { configMode=false }={}) {
    ...
    return sizes.map(s => `<span class="tag">${CONFIG.ROTV.actorSizes[s].label}</span>`).join("");
  }
