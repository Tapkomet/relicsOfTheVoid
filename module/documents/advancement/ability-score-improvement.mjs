...
export default class AbilityScoreImprovementAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      order: 20,
      icon: "systems/rotv/icons/svg/ability-score-improvement.svg",
      title: game.i18n.localize("ROTV.AdvancementAbilityScoreImprovementTitle"),
      hint: game.i18n.localize("ROTV.AdvancementAbilityScoreImprovementHint"),
      apps: {
   ...
   */
  get allowFeat() {
    return (this.item.type === "class") && game.settings.get("rotv", "allowFeats");
  }
   ...
   */
  canImprove(ability) {
    return CONFIG.ROTV.abilities[ability]?.improvement !== false;
  }
  ...
  titleForLevel(level, { configMode=false }={}) {
    ...
    return game.i18n.localize("ROTV.Feature.Feat");
  }
  ...
  summaryForLevel(level, { configMode=false }={}) {
    ...
    if ( configMode ) {
      const entries = Object.entries(this.configuration.fixed).map(([key, value]) => {
        ...
        const name = CONFIG.ROTV.abilities[key]?.label ?? key;
      ...
      if ( this.configuration.points ) entries.push(`<span class="tag">${
        game.i18n.localize("ROTV.AdvancementAbilityScoreImprovementPoints")}: <strong>${
