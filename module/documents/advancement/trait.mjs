...
export default class TraitAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      order: 30,
      icon: "systems/rotv/icons/svg/trait.svg",
      title: game.i18n.localize("ROTV.AdvancementTraitTitle"),
      hint: game.i18n.localize("ROTV.AdvancementTraitHint"),
      apps: {
        config: TraitConfig,
        flow: TraitFlow
      }
  ...
  prepareData() {
    ...
    const traitConfig = rep.size === 1 ? CONFIG.ROTV.traits[rep.first()] : null;
  ...
  sortingValueForLevel(levels) {
    const traitOrder = Object.keys(CONFIG.ROTV.traits).findIndex(k => k === this.representedTraits().first());
    const modeOrder = Object.keys(CONFIG.ROTV.traitModes).findIndex(k => k === this.configuration.mode);
  ...
  async actorSelected() {
    ...
    const traitTypes = this.configuration.mode === "default" ? Object.keys(CONFIG.ROTV.traits)
      : filteredKeys(CONFIG.ROTV.traits, t => t.expertise);
  ...
  async availableChoices(chosen) {
    ...
    if ( this.configuration.allowReplacements && (unfilteredLength > available.length) ) {
      ...
      return {
        ...
        label: game.i18n.format("ROTV.AdvancementTraitChoicesRemaining", {
