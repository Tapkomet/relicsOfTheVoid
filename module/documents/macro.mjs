 ...
 * @returns {Promise}
 */
export async function createRotVMacro(dropData, slot) {
  const macroData = { type: "script", scope: "actor" };
  switch ( dropData.type ) {
    case "Item":
      const itemData = await Item.implementation.fromDropData(dropData);
      if ( !itemData ) {
        ui.notifications.warn("MACRO.RotVUnownedWarn", {localize: true});
        return null;
      }
      foundry.utils.mergeObject(macroData, {
        name: itemData.name,
        img: itemData.img,
        command: `rotv.documents.macro.rollItem("${itemData._source.name}")`,
    ...
    case "ActiveEffect":
      ...
      foundry.utils.mergeObject(macroData, {
        ...
        img: effectData.icon,
        command: `rotv.documents.macro.toggleEffect("${effectData.name}")`,
...
function getMacroTarget(name, documentType) {
  ...
  if ( !actor ) {
    ui.notifications.warn("MACRO.RotVNoActorSelected", {localize: true});
    return null;
  }
  ...
  if ( documents.length === 0 ) {
    ui.notifications.warn(game.i18n.format("MACRO.RotVMissingTargetWarn", { actor: actor.name, type, name }));
    return null;
  }
  if ( documents.length > 1 ) {
    ui.notifications.warn(game.i18n.format("MACRO.RotVMultipleTargetsWarn", { actor: actor.name, type, name }));
  }
  return documents[0];
}
