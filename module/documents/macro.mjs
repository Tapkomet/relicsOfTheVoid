 ...
 */
export async function createRelicsMacro(dropData, slot) {
  const macroData = { type: "script", scope: "actor" };
  switch ( dropData.type ) {
    case "Item":
      const itemData = await Item.implementation.fromDropData(dropData);
      if ( !itemData ) return ui.notifications.warn(game.i18n.localize("MACRO.RelicsUnownedWarn"));
      foundry.utils.mergeObject(macroData, {
        name: itemData.name,
        img: itemData.img,
        command: `rotv.documents.macro.rollItem("${itemData.name}")`,
        flags: {"rotv.itemMacro": true}
      });
      break;
    case "ActiveEffect":
      const effectData = await ActiveEffect.implementation.fromDropData(dropData);
      if ( !effectData ) return ui.notifications.warn(game.i18n.localize("MACRO.RelicsUnownedWarn"));
      foundry.utils.mergeObject(macroData, {
        name: effectData.label,
        img: effectData.icon,
        command: `rotv.documents.macro.toggleEffect("${effectData.label}")`,
        flags: {"rotv.effectMacro": true}
      });
      break;
    default:
      return;
...
function getMacroTarget(name, documentType) {
  ...
  if ( !actor ) return ui.notifications.warn(game.i18n.localize("MACRO.RelicsNoActorSelected"));
  ...
  if ( documents.length === 0 ) {
    return ui.notifications.warn(game.i18n.format("MACRO.RelicsMissingTargetWarn", { actor: actor.name, type, name }));
  }
  if ( documents.length > 1 ) {
    ui.notifications.warn(game.i18n.format("MACRO.RelicsMultipleTargetsWarn", { actor: actor.name, type, name }));
  }
  return documents[0];
}
