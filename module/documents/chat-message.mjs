...
export default class ChatMessageRotV extends ChatMessage {
  ...
  get canSelectTargets() {
    if ( this.flags.rotv?.roll?.type !== "attack" ) return false;
  ...
  /** @inheritDoc */
  get isRoll() {
    return super.isRoll && !this.flags.rotv?.rest;
  }
  ...
  async getHTML(...args) {
    ...
    if ( game.settings.get("rotv", "autoCollapseItemCards") ) {
    ...
    /**
     * A hook event that fires after rotv-specific chat message modifications have completed.
     * @function rotv.renderChatMessage
     * @memberof hookEvents
     * @param {ChatMessageRotV} message  Chat message being rendered.
     * @param {HTMLElement} html       HTML contents of the message.
     */
    Hooks.callAll("rotv.renderChatMessage", this, html[0]);
  ...
  _collapseTrays(html) {
    let collapse;
    switch ( game.settings.get("rotv", "autoCollapseChatTrays") ) {
  ...
  _displayChatActionButtons(html) {
    const chatCard = html.find(".rotv.chat-card, .rotv2.chat-card");
    if ( chatCard.length > 0 ) {
      ...
      if ( game.user.isGM || actor?.isOwner || (this.user.id === game.user.id) ) {
        ...
        optionallyHide('button[data-action="consumeUsage"]', this.getFlag("rotv", "use.consumedUsage"));
        optionallyHide('button[data-action="consumeResource"]', this.getFlag("rotv", "use.consumedResource"));
        return;
      }
  ...
  _highlightCriticalSuccessFailure(html) {
    ...
    for ( let [index, d20Roll] of this.rolls.entries() ) {
      ...
      d20Roll = rotv.dice.D20Roll.fromRoll(d20Roll);
  ...
  _enrichChatCard(html) {
    ...
    anchor.setAttribute("aria-label", game.i18n.localize("ROTV.AdditionalControls"));
    ...
    // SVG icons
    html.querySelectorAll("i.rotv-icon").forEach(el => {
      const icon = document.createElement("rotv-icon");
      icon.src = el.dataset.src;
      el.replaceWith(icon);
    });

    // Enriched roll flavor
    const roll = this.getFlag("rotv", "roll");
    ...
    if ( this.isContentVisible && item ) {
      ...
      const subtitle = roll.type === "damage"
        ? isCritical ? game.i18n.localize("ROTV.CriticalHit") : game.i18n.localize("ROTV.DamageRoll")
        : roll.type === "attack"
          ? game.i18n.localize(`ROTV.Action${item.system.actionType.toUpperCase()}`)
          : item.system.type?.label ?? game.i18n.localize(CONFIG.Item.typeLabels[item.type]);
      const flavor = document.createElement("div");
      flavor.classList.add("rotv2", "chat-card");
      flavor.innerHTML = `
    ...
    tray.innerHTML = `
      <div class="card-tray targets-tray collapsible collapsed">
        <label class="roboto-upper">
          ...
          <span>${game.i18n.localize("ROTV.TargetPl")}</span>
          <i class="fas fa-caret-down" inert></i>
        </label>
        <div class="collapsible-content">
          <ul class="rotv2 unlist evaluation wrapper"></ul>
        </div>
      </div>
    `;
  ...
  _enrichDamageTooltip(rolls, html) {
    ...
    const aggregatedRolls = CONFIG.ROTV.aggregateDamageDisplay ? aggregateDamageRolls(rolls) : rolls;
  ...
  _enrichEnchantmentTooltip(html) {
    const enchantmentProfile = this.getFlag("rotv", "use.enchantmentProfile");
    if ( !enchantmentProfile ) return;

    // Ensure concentration is still being maintained
    const concentrationId = this.getFlag("rotv", "use.concentrationId");
    ...
    enchantmentApplication.classList.add("rotv2");
  ...
  static addChatMessageContextOptions(html, options) {
    ...
    options.push(
      {
        name: game.i18n.localize("ROTV.ChatContextDamage"),
      ...
      {
        name: game.i18n.localize("ROTV.ChatContextHealing"),
      ...
      {
        name: game.i18n.localize("ROTV.ChatContextTempHP"),
      ...
      {
        name: game.i18n.localize("ROTV.ChatContextDoubleDamage"),
      ...
      {
        name: game.i18n.localize("ROTV.ChatContextHalfDamage"),
      ...
      {
        name: game.i18n.localize("ROTV.ChatContextSelectHit"),
      ...
      {
        name: game.i18n.localize("ROTV.ChatContextSelectMiss"),
        icon: '<i class="fas fa-bullseye"></i>',
        condition: canTarget,
   ...
   * @returns {ItemRotV|void}
   */
  getAssociatedItem() {
    const actor = this.getAssociatedActor();
    if ( !actor ) return;
    const storedData = this.getFlag("rotv", "itemData");
    return storedData
      ? new Item.implementation(storedData, { parent: actor })
      : actor.items.get(this.getFlag("rotv", "use.itemId"));
  }
}
