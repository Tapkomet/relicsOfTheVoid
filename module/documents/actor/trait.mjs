 ...
 * @param {ActorRotV} actor  Actor from which to retrieve the values.
 * @param {string} trait   Trait as defined in `CONFIG.ROTV.traits`.
 ...
 * @param {string} [trait]  Trait as defined in `CONFIG.ROTV.traits`, only needed if key isn't prefixed.
 * @returns {string|void}
 */
...
export async function categories(trait) {
  const traitConfig = CONFIG.ROTV.traits[trait];
  const config = foundry.utils.deepClone(CONFIG.ROTV[traitConfig.configKey ?? trait]);
  ...
  for ( const key of Object.keys(config) ) {
    ...
    if ( traitConfig.children?.[key] ) {
      ...
      for ( const [childKey, value] of Object.entries(CONFIG.ROTV[traitConfig.children[key]]) ) {
  ...
  if ( traitConfig.subtypes ) {
    const map = CONFIG.ROTV[`${trait}ProficienciesMap`];

    // Merge all ID lists together
    const ids = traitConfig.subtypes.ids.reduce((obj, key) => {
      foundry.utils.mergeObject(obj, CONFIG.ROTV[key] ?? {});
      return obj;
    }, {});
 ...
 *                                       false.
 * @returns {Promise<ItemRotV>|object}     Promise for a `Document` if `indexOnly` is false & `fullItem` is true,
...
export function getBaseItem(identifier, { indexOnly=false, fullItem=false }={}) {
  ...
  // Full ItemRotV document required, always async.
...
export function traitIndexFields() {
  ...
  for ( const traitConfig of Object.values(CONFIG.ROTV.traits) ) {
...
export function traitLabel(trait, count) {
  ...
  if ( !traitConfig ) return game.i18n.localize(`ROTV.TraitGenericPlural.${pluralRule}`);
 ...
 * @param {string} [config.trait]   Trait as defined in `CONFIG.ROTV.traits` if not using a prefixed key.
...
export function keyLabel(key, config={}) {
  ...
  const traitData = CONFIG.ROTV[traitConfig.configKey ?? trait] ?? {};
  ...
  if ( lastKey === "*" ) {
    ...
    const localization = `ROTV.TraitConfigChoose${final ? "Other" : `Any${count ? "Counted" : "Uncounted"}`}`;
    ...
    for ( const childrenKey of Object.values(traitConfig.children ?? {}) ) {
      const childLabel = CONFIG.ROTV[childrenKey]?.[lastKey];
      if ( childLabel ) return childLabel;
    }

    // Base item (e.g. "Shortsword")
    for ( const idsKey of traitConfig.subtypes?.ids ?? [] ) {
      const baseItemId = CONFIG.ROTV[idsKey]?.[lastKey];
...
export function choiceLabel(choice, { only=false, final=false }={}) {
  ...
  return game.i18n.format("ROTV.TraitConfigChooseList", {
    count: choice.count,
...
export function localizedList({ grants=new Set(), choices=[] }) {
  ...
  return game.i18n.format("ROTV.TraitConfigChooseWrapper", {
