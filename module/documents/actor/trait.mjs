 ...
 * @param {string} trait  Trait as defined in `CONFIG.ROTV.traits`.
 * @returns {object}      Trait categories defined within `CONFIG.ROTV`.
 */
export function categories(trait) {
  const traitConfig = CONFIG.ROTV.traits[trait];
  return CONFIG.ROTV[traitConfig.configKey ?? trait];
}
...
export async function choices(trait, chosen=new Set()) {
  ...
  if ( traitConfig.children ) {
    for ( const [categoryKey, childrenKey] of Object.entries(traitConfig.children) ) {
      const children = CONFIG.ROTV[childrenKey];
  ...
  if ( traitConfig.subtypes ) {
    ...
    const ids = traitConfig.subtypes.ids.reduce((obj, key) => {
      if ( CONFIG.ROTV[key] ) Object.assign(obj, CONFIG.ROTV[key]);
      return obj;
    }, {});
  ...
  // Sort Categories
  if ( traitConfig.sortCategories ) data = rotv.utils.sortObjectEntries(data, "label");

  // Sort Children
  for ( const category of Object.values(data) ) {
    if ( !category.children ) continue;
    category.children = rotv.utils.sortObjectEntries(category.children, "label");
  }
...
export function traitLabel(trait, count) {
  ...
  return game.i18n.localize(`ROTV.Trait${typeCap}Plural.${pluralRule}`);
}
...
export function keyLabel(trait, key) {
  ...
  for ( const childrenKey of Object.values(traitConfig.children ?? {}) ) {
    if ( CONFIG.ROTV[childrenKey]?.[key] ) return CONFIG.ROTV[childrenKey]?.[key];
  }

  for ( const idsKey of traitConfig.subtypes?.ids ?? [] ) {
    if ( !CONFIG.ROTV[idsKey]?.[key] ) continue;
    const index = getBaseItem(CONFIG.ROTV[idsKey][key], { indexOnly: true });
...
export function choiceLabel(trait, choice) {
  ...
  if ( !choice.pool ) {
    return game.i18n.format("ROTV.TraitConfigChooseAny", {
      count: choice.count,
