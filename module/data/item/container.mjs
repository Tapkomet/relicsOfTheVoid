 ...
 * @property {string} capacity.type         Method for tracking max capacity as defined in `ROTV.itemCapacityTypes`.
...
export default class ContainerData extends ItemDataModel.mixin(
  ...
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      ...
      properties: new foundry.data.fields.SetField(new foundry.data.fields.StringField(), {
        label: "ROTV.ItemContainerProperties"
      }),
      capacity: new foundry.data.fields.SchemaField({
        type: new foundry.data.fields.StringField({
          required: true, initial: "weight", blank: false, label: "ROTV.ItemContainerCapacityType"
        }),
        value: new foundry.data.fields.NumberField({
          required: true, min: 0, label: "ROTV.ItemContainerCapacityMax"
        })
      }, {label: "ROTV.ItemContainerCapacity"})
    });
  }
  ...
  static _migrateWeightlessData(source) {
    if ( foundry.utils.getProperty(source, "system.capacity.weightless") === true ) {
      foundry.utils.setProperty(source, "flags.rotv.migratedProperties", ["weightlessContents"]);
    }
  }
   ...
   * @type {Collection<ItemRotV>|Promise<Collection<ItemRotV>>}
   */
  get contents() {
   ...
   * @returns {Promise<Collection<ItemRotV>>}
   * @private
   */
   ...
   * @returns {ItemRotV|Promise<ItemRotV>}  Item if found.
   */
  getContainedItem(id) {
  ...
  /**
   * @typedef {object} ItemRotVCapacityDescriptor
   ...
   * @returns {Promise<ItemRotVCapacityDescriptor>}
   */
  async computeCapacity() {
    ...
    if ( type === "weight" ) {
      ...
      context.units = game.i18n.localize("ROTV.AbbreviationLbs");
    } else {
      context.value = await this.contentsCount;
      context.units = game.i18n.localize("ROTV.ItemContainerCapacityItems");
    }
