...
export default class RaceData extends ItemDataModel.mixin(ItemDescriptionTemplate) {
  ...
  static get compendiumBrowserFilters() {
    return new Map([
      ["hasDarkvision", {
        label: "ROTV.CompendiumBrowser.Filters.HasDarkvision",
        type: "boolean",
   ...
   */
  get movementLabels() {
    const units = CONFIG.ROTV.movementUnits[this.movement.units || Object.keys(CONFIG.ROTV.movementUnits)[0]];
    return Object.entries(CONFIG.ROTV.movementTypes).reduce((obj, [k, label]) => {
   ...
   */
  get sensesLabels() {
    const units = CONFIG.ROTV.movementUnits[this.senses.units || Object.keys(CONFIG.ROTV.movementUnits)[0]];
    return Object.entries(CONFIG.ROTV.senses).reduce((arr, [k, label]) => {
   ...
   */
  get typeLabel() {
    return ActorRotV.formatCreatureType(this.type);
  }
  ...
  async _preCreate(data, options, user) {
    ...
    this.parent.updateSource({"system.advancement": toCreate.map(c => {
      const config = CONFIG.ROTV.advancementTypes[c.type];
