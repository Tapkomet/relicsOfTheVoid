...
export default class EquipmentData extends ItemDataModel.mixin(
  ...
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      type: new ItemTypeField({value: "light", subtype: false}, {label: "ROTV.ItemEquipmentType"}),
      armor: new SchemaField({
        value: new NumberField({required: true, integer: true, min: 0, label: "ROTV.ArmorClass"}),
        magicalBonus: new NumberField({min: 0, integer: true, label: "ROTV.MagicalBonus"}),
        dex: new NumberField({required: true, integer: true, label: "ROTV.ItemEquipmentDexMod"})
      }),
      properties: new SetField(new StringField(), {
        label: "ROTV.ItemEquipmentProperties"
      }),
      speed: new SchemaField({
        value: new NumberField({required: true, min: 0, label: "ROTV.Speed"}),
        conditions: new StringField({required: true, label: "ROTV.SpeedConditions"})
      }, {label: "ROTV.Speed"}),
  ...
  static get compendiumBrowserFilters() {
    return new Map([
      ["type", {
        label: "ROTV.ItemEquipmentType",
        type: "set",
        config: {
          choices: CONFIG.ROTV.equipmentTypes,
  ...
  static _migrateStealth(source) {
    if ( foundry.utils.getProperty(source, "system.stealth") === true ) {
      foundry.utils.setProperty(source, "flags.rotv.migratedProperties", ["stealthDisadvantage"]);
    }
  }
  ...
  prepareDerivedData() {
    ...
    this.type.label = CONFIG.ROTV.equipmentTypes[this.type.value]
  ...
  get chatProperties() {
    return [
      ...
      this.properties.has("stealthDisadvantage") ? game.i18n.localize("ROTV.Item.Property.StealthDisadvantage") : null
    ];
  }
   ...
   */
  get isArmor() {
    return this.type.value in CONFIG.ROTV.armorTypes;
  }
  ...
  get proficiencyMultiplier() {
    ...
    const config = CONFIG.ROTV.armorProficienciesMap;
