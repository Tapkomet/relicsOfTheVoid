...
export class EquipmentEntryData extends foundry.abstract.DataModel {
  ...
  static GROUPING_TYPES = {
    OR: "ROTV.StartingEquipment.Operator.OR",
    AND: "ROTV.StartingEquipment.Operator.AND"
  };
  ...
  static OPTION_TYPES = {
    // Category types
    armor: "ROTV.StartingEquipment.Choice.Armor",
    tool: "ROTV.StartingEquipment.Choice.Tool",
    weapon: "ROTV.StartingEquipment.Choice.Weapon",
    focus: "ROTV.StartingEquipment.Choice.Focus",

    // Generic item type
    linked: "ROTV.StartingEquipment.SpecificItem"
  };
  ...
  /**
   * Where in `CONFIG.ROTV` to find the type category labels.
  ...
  static defineSchema() {
    return {
      ...
      requiresProficiency: new BooleanField({label: "ROTV.StartingEquipment.Proficient.Label"})
    };
  }
  ...
  get label() {
    ...
    else if ( this.type !== "linked" ) label = game.i18n.format("ROTV.TraitConfigChooseAnyUncounted", { type: label });
    if ( (this.type === "linked") && this.requiresProficiency ) {
      label += ` (${game.i18n.localize("ROTV.StartingEquipment.IfProficient").toLowerCase()})`;
    }
    return label;
  }
  ...
  get categoryLabel() {
    ...
    if ( this.type === "weapon" ) label = game.i18n.format("ROTV.WeaponCategory", { category: label });
   ...
   */
  get keyOptions() {
    const config = foundry.utils.deepClone(CONFIG.ROTV[this.constructor.CATEGORIES[this.type]?.config]);
    if ( this.type === "weapon" ) foundry.utils.mergeObject(config, CONFIG.ROTV.weaponTypes);
