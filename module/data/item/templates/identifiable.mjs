...
export default class IdentifiableTemplate extends SystemDataModel {
  ...
  static defineSchema() {
    return {
      identified: new BooleanField({required: true, initial: true, label: "ROTV.Identified"}),
      unidentified: new SchemaField({
        name: new StringField({label: "ROTV.NameUnidentified"}),
        description: new HTMLField({label: "ROTV.DescriptionUnidentified"})
      })
    };
  }
  ...
  async _preUpdate(changed, options, user) {
    ...
    if ( this.parent.type === "weapon" ) baseItemIdentifier = CONFIG.ROTV.weaponIds[this.type.baseItem];
    else if ( this.parent.type === "tool" ) baseItemIdentifier = CONFIG.ROTV.toolIds[this.type.baseItem];
    else if ( this.parent.type === "equipment" ) {
      if ( this.type.value === "shield" ) baseItemIdentifier = CONFIG.ROTV.shieldIds[this.type.baseItem];
      else baseItemIdentifier = CONFIG.ROTV.armorIds[this.type.baseItem];
    }
    ...
    if ( baseItem ) {
      if ( fetchName ) {
        foundry.utils.setProperty(changed, "system.unidentified.name", game.i18n.format(
          "ROTV.Unidentified.DefaultName", { name: baseItem.name }
        ));
      }
      if ( fetchDesc ) {
    ...
    if ( fetchName ) foundry.utils.setProperty(changed, "system.unidentified.name", game.i18n.format(
      "ROTV.Unidentified.DefaultName", { name: game.i18n.localize(CONFIG.Item.typeLabels[this.parent.type]) }
    ));
  }
}
