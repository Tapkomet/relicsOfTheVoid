...
export class EnchantmentData extends foundry.abstract.DataModel {
   ...
   */
  get enchantments() {
    return this.item.effects.filter(ae => ae.getFlag("rotv", "type") === "enchantment");
  }
  ...
  static isEnchantmentSource(data) {
    return !this.isEnchantment(data)
      && CONFIG.ROTV.featureTypes[data.type?.value]?.subtypes?.[data.type?.subtype]
      && (data.type?.subtype in CONFIG.ROTV.featureTypes.enchantment.subtypes);
  }
   ...
   * @param {ItemRotV} item   Item from which to fetch the enchantments.
  ...
  static availableEnchantments(item) {
    ...
    return item.effects.filter(e => {
      if ( (e.getFlag("rotv", "type") !== "enchantment") || e.isAppliedEnchantment ) return false;
      const { min, max } = e.getFlag("rotv", "enchantment.level") ?? {};
   ...
   * @param {ItemRotV} item  Item that might be enchanted.
  ...
  canEnchant(item) {
    ...
    if ( !this.restrictions.allowMagical && item.system.properties?.has("mgc") ) {
      errors.push(new EnchantmentError(game.i18n.localize("ROTV.Enchantment.Warning.NoMagicalItems")));
    }

    if ( this.restrictions.type && (item.type !== this.restrictions.type) ) {
      errors.push(new EnchantmentError(game.i18n.format("ROTV.Enchantment.Warning.WrongType", {
