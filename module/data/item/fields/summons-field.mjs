...
export class SummonsData extends foundry.abstract.DataModel {
  ...
  static defineSchema() {
    return {
      bonuses: new SchemaField({
        ac: new FormulaField({
          label: "ROTV.Summoning.Bonuses.ArmorClass.Label", hint: "ROTV.Summoning.Bonuses.ArmorClass.hint"
        }),
        hd: new FormulaField({
          label: "ROTV.Summoning.Bonuses.HitDice.Label", hint: "ROTV.Summoning.Bonuses.HitDice.hint"
        }),
        hp: new FormulaField({
          label: "ROTV.Summoning.Bonuses.HitPoints.Label", hint: "ROTV.Summoning.Bonuses.HitPoints.hint"
        }),
        attackDamage: new FormulaField({
          label: "ROTV.Summoning.Bonuses.Attack.Label", hint: "ROTV.Summoning.Bonuses.Attack.Hint"
        }),
        saveDamage: new FormulaField({
          label: "ROTV.Summoning.Bonuses.Saves.Label", hint: "ROTV.Summoning.Bonuses.Saves.Hint"
        }),
        healing: new FormulaField({
          label: "ROTV.Summoning.Bonuses.Healing.Label", hint: "ROTV.Summoning.Bonuses.Healing.Hint"
        })
      }),
      classIdentifier: new IdentifierField(),
      creatureSizes: new SetField(new StringField(), {
        label: "ROTV.Summoning.CreatureSizes.Label", hint: "ROTV.Summoning.CreatureSizes.Hint"
      }),
      creatureTypes: new SetField(new StringField(), {
        label: "ROTV.Summoning.CreatureTypes.Label", hint: "ROTV.Summoning.CreatureTypes.Hint"
      }),
      match: new SchemaField({
        attacks: new BooleanField({
          label: "ROTV.Summoning.Match.Attacks.Label", hint: "ROTV.Summoning.Match.Attacks.Hint"
        }),
        proficiency: new BooleanField({
          label: "ROTV.Summoning.Match.Proficiency.Label", hint: "ROTV.Summoning.Match.Proficiency.Hint"
        }),
        saves: new BooleanField({
          label: "ROTV.Summoning.Match.Saves.Label", hint: "ROTV.Summoning.Match.Saves.Hint"
        })
      }),
      mode: new StringField({label: "ROTV.Summoning.Mode.Label", hint: "ROTV.Summoning.Mode.Hint"}),
      ...
      prompt: new BooleanField({
        initial: true, label: "ROTV.Summoning.Prompt.Label", hint: "ROTV.Summoning.Prompt.Hint"
      })
    };
  }
  ...
  static get canSummon() {
    return game.user.can("TOKEN_CREATE") && (game.user.isGM || game.settings.get("rotv", "allowSummoning"));
  }
  ...
  get summonedCreatures() {
    ...
    return SummonsData.summonedCreatures(this.item.actor)
      .filter(i => i?.getFlag("rotv", "summon.origin") === this.item.uuid);
  }
  ...
  async summon(profileId, options={}) {
    ...
    if ( !profile ) {
      throw new Error(game.i18n.format("ROTV.Summoning.Warning.NoProfile", { profileId, item: this.item.name }));
    }

    /**
     * A hook event that fires before summoning is performed.
     * @function rotv.preSummon
     * @memberof hookEvents
     * @param {ItemRotV} item               The item that is performing the summoning.
     ...
     */
    if ( Hooks.call("rotv.preSummon", this.item, profile, options) === false ) return;
    ...
    if ( !actor.isOwner ) {
      throw new Error(game.i18n.format("ROTV.Summoning.Warning.NoOwnership", { actor: actor.name }));
    }
    ...
    try {
      ...
      for ( const placement of placements ) {
         ...
         */
        if ( Hooks.call("rotv.preSummonToken", this.item, profile, tokenUpdateData, options) === false ) continue;
         ...
         * @param {SummoningOptions} options  Additional summoning options.
         */
        Hooks.callAll("rotv.summonToken", this.item, profile, tokenData, options);
     ...
     * @function rotv.postSummon
     * @memberof hookEvents
     * @param {ItemRotV} item               The item that is performing the summoning.
     * @param {SummonsProfile} profile    Profile used for summoning.
     * @param {TokenRotV[]} tokens          Tokens that have been created.
     * @param {SummoningOptions} options  Additional summoning options.
     */
    Hooks.callAll("rotv.postSummon", this.item, profile, createdTokens, options);
  }
   ...
   * @returns {ActorRotV}    Local copy of actor.
   */
  async fetchActor(uuid) {
    const actor = await fromUuid(uuid);
    if ( !actor ) throw new Error(game.i18n.format("ROTV.Summoning.Warning.NoActor", { uuid }));

    const actorLink = actor.prototypeToken.actorLink;
    if ( !actor.pack && (!actorLink || actor.getFlag("rotv", "summon.origin") === this.item.uuid )) return actor;
    ...
    const localActor = game.actors.find(a =>
      ...
      a.getFlag("rotv", "summonedCopy")
      // Sourced from the desired actor UUID
      && (a.getFlag("core", "sourceId") === uuid)
      // Unlinked or created from this item specifically
      && ((a.getFlag("rotv", "summon.origin") === this.item.uuid) || !a.prototypeToken.actorLink)
    );
    if ( localActor ) return localActor;

    // Check permissions to create actors before importing
    if ( !game.user.can("ACTOR_CREATE") ) throw new Error(game.i18n.localize("ROTV.Summoning.Warning.CreateActor"));
   ...
   * @param {ActorRotV} actor             Actor that will be modified.
  ...
  async getChanges(actor, profile, options) {
    ...
    // Add flags
    actorUpdates["flags.rotv.summon"] = {
    ...
    if ( this.match.proficiency ) {
      const proficiencyEffect = new ActiveEffect({
        _id: staticID("rotvMatchProficiency"),
        changes: [{
        ...
        name: game.i18n.localize("ROTV.Summoning.Match.Proficiency.Label")
      });
      ...
      if ( acBonus.total ) {
        if ( actor.system.attributes.ac.calc === "flat" ) {
          ...
          actorUpdates.effects.push((new ActiveEffect({
            ...
            name: game.i18n.localize("ROTV.Summoning.Bonuses.ArmorClass.Label")
          })).toObject());
        }
      }
    }
    ...
    if ( this.bonuses.hd && (actor.type === "npc") ) {
      ...
      if ( hdBonus.total ) {
        actorUpdates.effects.push((new ActiveEffect({
          ...
          name: game.i18n.localize("ROTV.Summoning.Bonuses.HitDice.Label")
        })).toObject());
      }
    }
      ...
      if ( hpBonus.total ) {
        ...
        const maxHpEffect = hpField => {
          return (new ActiveEffect({
            ...
            name: game.i18n.localize("ROTV.Summoning.Bonuses.HitPoints.Label")
          })).toObject();
        };
    ...
    if ( this.creatureSizes.size ) {
      ...
      const config = CONFIG.ROTV.actorSizes[size];
      if ( config ) {
    ...
    for ( const item of actor.items ) {
      ...
      if ( changes.length ) {
        const effect = (new ActiveEffect({
          _id: staticID("rotvItemChanges"),
          changes,
          disabled: false,
          icon: "icons/skills/melee/strike-slashes-orange.webp",
          name: game.i18n.localize("ROTV.Summoning.ItemChanges.Label"),
          origin: this.item.uuid,
          flags: {
            rotv: { type: "enchantment" }
          }
        })).toObject();
   ...
   * @property {ActorRotV} actor            Original actor from which the token will be created.
  ...
  async getTokenData({ actor, placement, tokenUpdates, actorUpdates }) {
    if ( actor.prototypeToken.randomImg && !game.user.can("FILES_BROWSE") ) {
      ...
      ui.notifications.warn("ROTV.Summoning.Warning.Wildcard", { localize: true });
    }
    ...
    if ( profile.name ) label = profile.name;
      ...
      switch ( this.mode ) {
        case "cr":
          ...
          label = game.i18n.format("ROTV.Summoning.Profile.ChallengeRatingLabel", { cr: formatCR(cr) });
          break;
        default:
   ...
   * @param {ActorRotV} actor  Actor for which to find the summoned creatures.
   * @returns {ActorRotV[]}
   */
