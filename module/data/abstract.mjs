...
export default class SystemDataModel extends foundry.abstract.TypeDataModel {
   ...
   * @property {typeof DataModel} [systemFlagsModel]  Model that represents flags data within the rotv namespace.
   */
  ...
  static defineSchema() {
    ...
    for ( const template of this._schemaTemplates ) {
      if ( !template.defineSchema ) {
        throw new Error(`Invalid rotv template mixin ${template} defined on class ${this.constructor}`);
      }
...
export class ItemDataModel extends SystemDataModel {
   ...
   * @type {string}
   */
  static ITEM_TOOLTIP_TEMPLATE = "systems/rotv/templates/items/parts/item-tooltip.hbs";
  ...
  prepareBaseData() {
    if ( this.parent.isEmbedded ) {
      const sourceId = this.parent.flags.rotv?.sourceId ?? this.parent._stats.compendiumSource
  ...
  async richTooltip(enrichmentOptions={}) {
    return {
      ...
      classes: ["rotv2", "rotv-tooltip", "item-tooltip"]
    };
  }
  ...
  async getCardData(enrichmentOptions={}) {
    ...
    const context = {
      ...
      config: CONFIG.ROTV,
      controlHints: game.settings.get("rotv", "controlHints"),
    ...
    if ( context.labels.duration ) {
      context.labels.concentrationDuration = properties?.has("concentration")
        ? game.i18n.format("ROTV.ConcentrationDuration", {
  ...
  /**
   * @typedef {object} FavoriteDataRotV
