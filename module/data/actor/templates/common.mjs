...
export default class CommonTemplate extends ActorDataModel.mixin(CurrencyTemplate) {
    ...
    return this.mergeSchema(super.defineSchema(), {
      abilities: new MappingField(new foundry.data.fields.SchemaField({
        value: new foundry.data.fields.NumberField({
          required: true, nullable: false, integer: true, min: 0, initial: 0, label: "ROTV.AbilityScore"
        }),
        proficient: new foundry.data.fields.NumberField({
          required: true, integer: true, min: 0, max: 1, initial: 0, label: "ROTV.ProficiencyLevel"
        }),
        max: new foundry.data.fields.NumberField({
          required: true, integer: true, nullable: true, min: 0, initial: null, label: "ROTV.AbilityScoreMax"
        }),
        ...
        initialKeys: CONFIG.ROTV.abilities, initialValue: this._initialAbilityValue.bind(this),
        initialKeysOnly: true, label: "ROTV.Abilities"
      })
    });
  }
  ...
  static _initialAbilityValue(key, initial, existing) {
    const config = CONFIG.ROTV.abilities[key];
    if ( config ) {
  ...
  prepareAbilities({ rollData={}, originalSaves }={}) {
    const flags = this.parent.flags.rotv ?? {};
    ...
    for ( const [id, abl] of Object.entries(this.abilities) ) {
      ...
      if ( !Number.isFinite(abl.max) ) abl.max = CONFIG.ROTV.maxAbilityScore;
