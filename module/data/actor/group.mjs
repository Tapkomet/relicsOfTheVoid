 ...
 * @property {ActorRotV} actor              Associated actor document.
...
/**
 * A data model and API layer which handles the schema and functionality of "group" type Actors in the rotv system.
 ...
 * const g = new rotv.documents.ActorRotV({
 *  type: "group",
 *  name: "Test Group",
 *  system: {
...
export default class GroupActor extends ActorDataModel.mixin(CurrencyTemplate) {
  ...
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      type: new SchemaField({
        value: new StringField({initial: "party", label: "ROTV.Group.Type"})
      }),
      description: new SchemaField({
        full: new HTMLField({label: "ROTV.Description"}),
        summary: new HTMLField({label: "ROTV.DescriptionSummary"})
      }),
      members: new ArrayField(new SchemaField({
        actor: new ForeignDocumentField(foundry.documents.BaseActor),
        quantity: new SchemaField({
          value: new NumberField({initial: 1, integer: true, min: 0, label: "ROTV.Quantity"}),
          formula: new FormulaField({label: "ROTV.QuantityFormula"})
        })
      }), {label: "ROTV.GroupMembers"}),
      attributes: new SchemaField({
        movement: new SchemaField({
          land: new NumberField({nullable: false, min: 0, step: 0.1, initial: 0, label: "ROTV.MovementLand"}),
          water: new NumberField({nullable: false, min: 0, step: 0.1, initial: 0, label: "ROTV.MovementWater"}),
          air: new NumberField({nullable: false, min: 0, step: 0.1, initial: 0, label: "ROTV.MovementAir"})
        })
      }, {label: "ROTV.Attributes"}),
      details: new SchemaField({
        xp: new foundry.data.fields.SchemaField({
          value: new foundry.data.fields.NumberField({
            integer: true, min: 0, label: "ROTV.ExperiencePointsCurrent"
          })
   ...
   * @param {ActorRotV} actor           A non-group Actor to add to the group
   * @returns {Promise<ActorRotV>}      The updated group Actor
   */
   ...
   * @param {ActorRotV|string} actor    An Actor or ID to remove from this group
   * @returns {Promise<ActorRotV>}      The updated group Actor
   */
  ...
  async rest(config, result) {
     ...
     * @function rotv.groupRestCompleted
     * @memberof hookEvents
     * @param {ActorRotV} group                         The group that just completed resting.
     * @param {Map<ActorRotV, RestResult|null>} result  Details on the rests completed.
     */
    Hooks.callAll("rotv.groupRestCompleted", this.parent, results);
  ...
  _onUpdate(changed, options, userId) {
    if ( !foundry.utils.hasProperty(changed, "system.type.value") || (game.user !== game.users.activeGM)
      || (game.settings.get("rotv", "primaryParty")?.actor !== this.parent)
