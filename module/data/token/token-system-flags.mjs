  ...
  static defineSchema() {
    return {
      ...
      tokenRing: new SchemaField({
        enabled: new BooleanField({label: "ROTV.TokenRings.Enabled"}),
        colors: new SchemaField({
          ring: new ColorField({required: false, label: "ROTV.TokenRings.RingColor"}),
          background: new ColorField({required: false, label: "ROTV.TokenRings.RingColor"})
        }, {required: false, initial: undefined}),
        effects: new NumberField({
          initial: 1, min: 0, max: 8388607, integer: true, label: "ROTV.TokenRings.Effects.Label"
        }),
        scaleCorrection: new NumberField({
          required: false, initial: 1, min: 0, label: "ROTV.TokenRings.ScaleCorrection"
        }),
        textures: new SchemaField({
          subject: new FilePathField({
            required: false, categories: ["IMAGE"], label: "ROTV.TokenRings.Subject.Label",
            hint: "ROTV.TokenRings.Subject.Hint"
          })
        }, {required: false, initial: undefined})
      }, {required: false, initial: undefined, label: "ROTV.TokenRings.Title"})
    };
  }
}
